...
もともとGemは有料の「Gemini Advanced」(現在のGoogle AI Pro相当)などの契約者限定機能だったが、2025年3月にGoogle公式ブログで無料ユーザーへの展開が発表され、現在は無料プランでもGemの作成・利用ができる([Google公式ブログ](https://blog.google/products/gemini/new-gemini-app-features-march-2025/)、[9to5Google](https://9to5google.com/2025/03/25/gemini-gems-free-mobile/))。ネット上の解説記事には「Gemはpro以上の機能」と書かれたものも残っているが、これは古い情報か、法人向けプランの話と混同している可能性が高い。無料プランと有料プランの違いは「作れるかどうか」ではなく、**そのGemが動くモデルの性能**にある。無料プランのGemは軽量モデル(Flash系)で動作し、有料プランでは同じGemがより高性能なモデル(Pro系)で動くという違いになる。

Gemと紛らわしいのが「**保存済み情報(Saved info)**」という機能である。両者は目的も挙動もまったく違う。
...
| 管理画面 | Gemマネージャー | gemini.google.com/saved-info(Web版のみ) |
...
| 手元の特定資料**だけ**を根拠に正確に調べたい(社内マニュアルなど) | NotebookLM([Google Geminiの基本](./google-gemini-basics.md)を参照)。GemのナレッジもRAG(検索拡張生成)的に使えるが、正確な出典追跡が主目的ならNotebookLMの方が向く |
...
| ナレッジ(任意) | 端末からのファイルアップロードまたはGoogle Driveのファイルを紐づけられる。同時に追加できるファイルは最大10個、動画は1本2GBまで、その他のファイル形式は1つ100MBまでが目安([Google公式ヘルプ: Gemini アプリでファイルをアップロードして分析する](https://support.google.com/gemini/answer/14903178?hl=ja))。Google Driveのファイルを参照させた場合は常に最新版が参照される |

4. 右側のプレビューでテスト対話をしながら内容を調整し、「保存」をクリック

作成したGemはGeminiのWeb版だけでなく、モバイルアプリやGoogle Workspaceのサイドパネルからも呼び出せる。
...
| 設定場所 | gemini.google.com左メニュー「Gemを表示」→Gemマネージャー | ChatGPT左サイドバー「GPTを探す」→GPT Builder | claude.ai/projects | Microsoft 365 Copilotアプリ左ペインの「エージェントの作成」 |
| 必要プラン | 無料プランでも作成・利用可(動作モデルの性能は有料プランほど高い) | Freeプランでは作成不可(利用のみ)。作成にはGo/Plus以上が必要 | Freeでも作成可(利用量は少ない)。本格利用はPro以上 | Copilot ChatまたはMicrosoft 365 Copilotライセンスが必要。社内データ(SharePoint等)との連携には追加のライセンス条件がある |
...
- **Gemは会話の内容を自動で学習しない**: Gemの指示文は自分で編集しない限り変わらない。会話の中で自然に個人情報を蓄積してほしい場合は、保存済み情報(Saved info)側の役割であり、両者を混同すると「毎回同じ質問をしているのに覚えてくれない」という誤解につながる
...
- **ファイルの追加数・サイズには上限がある**: 一度に追加できるナレッジファイルは目安として10個までで、ファイル形式ごとにサイズ上限がある。大量の社内資料を横断的に検索させたい用途には、Gemより NotebookLM や Google Workspace側の検索機能の方が向く場合がある
...
- [ChatGPTのメモリ(Memory)機能](../part02-chatgpt-basics/chatgpt-memory-feature.md)

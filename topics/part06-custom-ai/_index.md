---
title: "Part 6: カスタムAIの作成"
part: 6
---

# Part 6: カスタムAIの作成

GPTs(ChatGPT)、Gem(Gemini)、Copilot Agent、Claude Projects など、
自分専用のAIボットを作る方法を**ツール横断**で扱う。

## 章構成(分類の地図)

### 第1章 カスタムAIの基礎
- 共通の設計原則(指示・ナレッジ・公開範囲の考え方)

### 第2章 主要ツールでの作り方
- GPTs / Gem / Copilot Agent / Claude Projects の対応付け・使い分け

### 第3章 高度な活用と防御
- ナレッジファイル、アクション連携、プロンプトインジェクション対策

## 収録ページ

- [GPTsの作り方と公開設定](gpts-creation-basics.md)
- [Gem(Geminiのカスタムボット機能)の基本](gemini-gem-feature.md)
- [Claude(Anthropic)の「プロジェクト」機能の基本](claude-projects-basics.md)
- [GPTsのナレッジファイルとアクション連携](gpts-knowledge-and-actions.md)
- [GPTsにおけるプロンプトインジェクション対策](gpts-prompt-injection-defense.md)
- [Gem・Claude Projects・Copilotエージェントのプロンプトインジェクション対策](custom-ai-cross-tool-prompt-injection-defense.md)

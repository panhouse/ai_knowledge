...
> **2026年9月時点の重要な動き(GPTsは新規作成が事実上終了し、既存GPTsの保守運用フェーズに入った)**: OpenAIは2026年4月22日、GPTsの延長線上にある新機能「**ワークスペースエージェント(workspace agents)**」をChatGPT Business/Enterprise/Edu/Teachers向けに発表した。Codex(コード生成AI)を基盤にクラウド上で常時稼働し、Slack・Salesforce・SharePointなどの業務ツールに直接つながって複数ステップの作業を人手を介さず実行できる点がGPTsとの大きな違いである。これに伴いGPTsの終了スケジュールが具体化しており、**個人向け(Free/Go/Plus/Pro)アカウントは2026年8月16日付けで新規GPTの作成・GPT Storeへの公開ができなくなった**(既存GPTsの利用・編集は引き続き可能で、OpenAIは「当面の間」使えるとしている)。**法人向け(Business/Enterprise/Edu/Teachers)は、ワークスペースエージェントへの移行ツールが2026年9月17日頃に提供開始、新規GPT作成の終了が2026年9月25日頃、既存GPTsの完全停止(退役)が2026年12月11日と案内されており(Enterprise向けの公表スケジュール。ワークスペースにより前後する可能性がある)、本ページの内容は今後「新規に何かを作る」よりも「既存GPTsを保守運用する」ための知識としての比重が大きくなる。法人でGPTsを運用している場合は、Enterprise/Business管理画面の案内で自組織の移行期限を確認し、退役日までにワークスペースエージェントまたは後述のApps(MCPベース)への移行計画を立てておく必要がある。
...
- **Actionsと「Apps」は同じGPTに同時に持たせられない**: OpenAIは2025年にMCP(Model Context Protocol、外部ツール・データ接続の共通規格)をベースにした「**Apps**」(Apps SDKで開発する、対話中にUIカードを表示できる拡張機能)を導入した。1つのGPTには、従来のOpenAPIベースのActions、または新しいMCPベースのAppsの**どちらか一方**しか接続できない(2026年9月時点)。既存GPTのActionsをそのままMCPサーバーに置き換えることはできず、MCPサーバーに直接つなぎたい場合はActionsを外してAppsとして作り直すか、ChatGPTの「開発者モード(Developer Mode)」でMCPサーバーを個別に接続する(こちらはGPT本体ではなく通常のチャットに対する接続で、会話中はメモリ機能がオフになる)という別の経路になる。「GPTsでMCPサーバーをそのまま使いたい」という要望はコミュニティで多く上がっているが、Actions設定画面からMCPサーバーURLを直接登録する機能は2026年9月時点でも用意されていない。
...
もう1つの判断軸は「個人利用か、組織で作って配布・運用するか」。2026年9月時点では、この軸の意味合いが変わってきている。

- **個人(Free/Go/Plus/Pro)**: 2026年8月16日以降、新規GPTの作成・GPT Storeへの公開はできない。すでに作成済みのGPTを引き続き使う・編集するだけなら当面問題ないが、「これから新しくGPTsを作って始める」という選択肢自体が個人向けにはほぼ閉じている。新規に何かを作りたい場合は、通常のチャットへのファイル添付・Projects(プロジェクト機能)や、後述のAppsでの代替を検討する
- **組織(Business/Enterprise/Edu/Teachers)**: 新規GPT作成も2026年9月25日頃に終了し、既存GPTsも2026年12月11日(Enterprise向け公表スケジュール)に完全停止する見込み。「チーム全体が使う常時稼働の自動化」(定期実行・Slack通知・複数システムを跨いだ処理など)を組織として構築・運用したい場合は、ワークスペースエージェントへの移行を前提に設計するほうが将来的な手戻りが少ない

つまり本ページのナレッジファイル・Actionsの知識は、これから新しい仕組みを作るための情報としてよりも、**すでに稼働している既存GPTsを退役日まで保守・改修するための情報**として使う場面が中心になっていく。
...
2026年9月時点の上限の目安は次のとおり(2026年7月時点から変更なしを確認済み)。
...
3. 「Schema」欄に、呼び出したいAPIのOpenAPIスキーマ(JSONまたはYAML、`openapi`プロパティは`3.0`または`3.1.0`を指定)を貼り付ける。スキーマが正しければ、検出されたエンドポイントが一覧表示される
4. 一般公開するGPTの場合はプライバシーポリシーのURL登録が必須になる
5. 保存後、テスト会話でActionsが意図通り呼び出されるか確認する

OAuthを選ぶ場合は、GPT編集画面でクライアントID・クライアントシークレット・認可URL・トークンURL・スコープを入力する「認可コードフロー(Authorization Code)」のみに対応しており、利用者ごとにサインインさせて個別の権限で動かす用途に向く。一方、client credentials(クライアント資格情報)・サービスアカウント・JWTベアラー方式のようなユーザーを介さないマシン間(machine-to-machine)OAuthや、カスタムAPIキーの動的な差し替え、顧客側で用意したmTLS証明書の提示には2026年9月時点でも対応していない。こうした認証方式が必要なAPIをつなぐ場合は、API側に薄いプロキシ(認証を仲介する中継サーバー)を用意し、GPTsからは単純なAPIキー認証で呼び出す構成にするのが実務上の回避策になる。
...
- **「新規に作れる」機能ではなくなりつつあることを前提に運用する**: 2026年8月16日以降、個人アカウント(Free/Go/Plus/Pro)は新規GPTの作成・公開ができない。組織アカウント(Business/Enterprise/Edu/Teachers)も新規作成の終了(2026年9月25日頃)・既存GPTsの完全停止(2026年12月11日、Enterprise向け公表スケジュール)が予定されている。ナレッジファイルの追加・Actionsの改修といった本ページの手順は「既存GPTの保守」としては引き続き有効だが、法人でGPTsに業務を依存させている場合は、退役日より十分前に後継のワークスペースエージェントやAppsへの移行検証を始めること。スケジュールはワークスペースや発表時期により前後しうるため、実際の移行期限はChatGPT管理画面・Help Centerの案内を優先して確認する
...
自分または所属組織がGPTsを業務で使っている場合、まず今の自分のプラン・ワークスペースで新規GPTの作成が可能かをGPT編集画面で確認し、既存GPTには1つだけナレッジファイルを追加して内容の一部をQ&A形式に書き換え、回答の的確さが変わるか試してみる。組織でGPTsに依存した業務がある場合は、あわせて自組織の移行期限をChatGPT管理画面で確認しておく。
...
### 2026-09-13: GPTs終了スケジュールの具体化・Actions/Apps(MCP)の関係・OAuth制約を追記
- **内容**: 個人アカウント(Free/Go/Plus/Pro)が2026年8月16日付けで新規GPT作成・GPT Store公開ができなくなったこと、組織アカウント(Business/Enterprise/Edu/Teachers)も新規作成終了(2026年9月25日頃)・既存GPTs完全停止(2026年12月11日、Enterprise向け公表スケジュール)が予定されていることを反映し、冒頭の重要な動き・使いどころ・注意点を「既存GPTsの保守運用」中心の記述に更新。仕組み・背景に、1つのGPTには従来のOpenAPIベースActionsとMCPベースの新機能「Apps」のどちらか一方しか接続できないこと、GPTsのActions設定画面からMCPサーバーを直接登録する機能は2026年9月時点でも用意されていないことを追加。Actions設定手順にOAuthが認可コードフローのみ対応で、client credentialsなどマシン間OAuthやカスタムAPIキー・mTLS証明書の提示には対応していない点を追記。ナレッジファイルのアップロード上限(20ファイル/512MB/200万トークン等)は2026年9月時点でも変更がないことを再確認
- **出典**: [OpenAI: Introducing workspace agents in ChatGPT](https://openai.com/index/introducing-workspace-agents-in-chatgpt/)、[VentureBeat: OpenAI unveils Workspace Agents, a successor to custom GPTs](https://venturebeat.com/orchestration/openai-unveils-workspace-agents-a-successor-to-custom-gpts-for-enterprises-that-can-plug-directly-into-slack-salesforce-and-more)、[OpenAI Help Center: Creating and editing GPTs](https://help.openai.com/en/articles/8554397-creating-and-editing-gpts)、[OpenAI Help Center: Managing workspace lifecycle and migration in ChatGPT Business](https://help.openai.com/en/articles/8801890-managing-workspace-lifecycle-and-migration-in-chatgpt-business)、[OpenAI Help Center: ChatGPT Enterprise & Edu Release Notes](https://help.openai.com/en/articles/10128477-chatgpt-enterprise-edu-release-notes)、[sqmagazine: OpenAI Blocks Personal Accounts From Building New GPTs](https://sqmagazine.co.uk/openai-blocks-personal-accounts-new-gpts/)、[OpenAI Help Center: Apps in ChatGPT and the Apps SDK](https://help.openai.com/en/articles/12503483-apps-in-chatgpt-and-the-apps-sdk)、[OpenAI Developer Community: Support for MCP server URLs in custom GPTs](https://community.openai.com/t/support-for-mcp-server-urls-in-custom-gpts/1366998)、[OpenAI API: GPT Action authentication](https://developers.openai.com/api/docs/actions/authentication)、[OpenAI Help Center: File uploads FAQ](https://help.openai.com/en/articles/8555545-file-uploads-faq)

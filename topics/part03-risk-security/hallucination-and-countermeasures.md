...
- **「賢いモデル」ほど安全とは限らない**: 複雑な推論を行う推論モデル(o3など)は、事実に関する質問で知識の空白を推測で埋めやすく、OpenAI自身のPersonQAベンチマークでo3は約33%と、単純なモデルより高い幻覚率を示した(後継のGPT-5系では大幅に改善)。モデルの世代やタスクによって幻覚率は大きく変動するため、「新しいモデルだから間違えない」という思い込みは禁物。
...
- **架空判例の提出による制裁は現実に多発している**: 2023年にはニューヨークの弁護士がChatGPTに生成させた実在しない判例6件を法廷に提出し、5,000ドルの制裁金と裁判所への謝罪状送付を命じられた(Mata v. Avianca事件)。2026年5月にはオレゴン州の弁護士2名が23件の架空引用を含む書面を提出し、米国の同種事案で過去最高額となる合計11万ドルの制裁を受けた。同様の制裁事例は2023年以降だけで200件以上報告されており、米国では2026年時点で少なくとも25の連邦地裁がAI利用の申告と人間によるレビューを義務付ける標準命令を導入している。

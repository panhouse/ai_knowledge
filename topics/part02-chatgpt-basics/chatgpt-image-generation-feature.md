...
2026年4月には後継の「GPT Image 2」(消費者向けの呼び名は「ChatGPT Images 2.0」)が登場し、以下の点が強化された。

- 解像度が最大2K相当まで対応し、アスペクト比(縦横比)の指定範囲も拡大
- 日本語・韓国語・ヒンディー語など非ラテン文字系言語の文字描写精度が向上(以前は英語以外の文字が崩れやすいという弱点があった)
- 有料プラン限定の「Thinking(思考)」モードを使うと、1回の指示から人物・物体の見た目を保った状態で複数枚(最大8枚程度)をまとめて生成できる
...
| プラン | 画像生成 | 目安の上限 |
|---|---|---|
| Free(無料) | 利用可能(制限あり) | 1日あたり2〜3枚程度。上限に達すると一定時間待つか有料プランへの案内が出る |
| Go(月額$8程度) | 利用可能 | Freeより緩いが、Plus以上ほど自由度は高くない |
| Plus(月額$20) | 利用可能。Thinkingモードも使用可 | ローリングウィンドウ(直近の利用状況に応じて変動する)方式で、目安として3時間あたり数十枚程度 |
| Pro(月額$100/$200) | 利用可能。Thinkingモードも使用可 | 実質無制限(高速・高品質。OpenAIの不正利用防止ガードレールの範囲内) |
| Business・Enterprise | 利用可能 | 契約内容・管理設定による |

正確な上限値は変動しやすいため、契約前に[ChatGPT公式の料金ページ](https://chatgpt.com/pricing)で最新情報を確認すること。プラン全体の違いは[ChatGPTのプラン比較](chatgpt-plan-comparison.md)を参照。
...
- **無料プランは枚数制限がすぐ来る**: 1日2〜3枚程度で上限に達するため、業務で日常的に使うならPlus以上への切り替えを検討する
...
- [生成AIの著作権リスクと実務での注意点](../part03-risk-security/copyright-risks-in-generative-ai.md)
- [生成AIによるプレゼン資料・ドキュメント作成の実務活用](../part10-business-practice/ai-presentation-and-document-creation.md)

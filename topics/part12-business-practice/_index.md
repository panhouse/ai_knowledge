---
title: "Part 12: 業務活用・実践"
part: 12
---

# Part 12: 業務活用・実践

## 章構成(分類の地図)

### 第1章 導入の設計
- 業務切り分け、BPR、ROI測定、社内展開
- Human in the Loop(人間が最終確認・介在する業務設計)
- 生成AI利用コストの管理・予算配分(トークン課金の理解、部門別予算・社内チャージバック、コストの可視化)。
  [ROI測定・効果測定の考え方](ai-roi-measurement.md)とは目的が異なる(あちらは効果測定、こちらはコスト管理・予算配分)ことを明記して書く

### 第2章 文章・コミュニケーション
- 文章作成・編集、メール、翻訳、議事録

### 第3章 資料・ビジュアル作成
- 資料作成、スライド、画像

### 第4章 情報収集・リサーチ
- Deep Research、競合調査、要約

### 第5章 職種別ユースケースの入口
- 職種別の具体的な事例・業務単位の掘り下げは [Part 15: 職種別 生成AI活用事例](../part15-job-role-cases/_index.md) に移設した
- このパートで職種の話を扱うのは「職種を問わない手法」に整理できる場合のみ。
  特定職種の業務フローに踏み込む内容は Part 15 に書く

## 収録ページ

- [生成AIに向く業務・向かない業務の切り分け](ai-task-suitability.md)
- [Human in the Loop(人間参加型)の業務設計](human-in-the-loop-basics.md)
- [生成AIによる文章作成・編集の実務活用](ai-writing-and-editing.md)
- [生成AIによるBPR(業務プロセス再構築)](generative-ai-bpr.md)
- [生成AI導入のROI測定・効果測定の考え方](ai-roi-measurement.md)
- [生成AI利用コストの管理・予算配分](ai-cost-management-and-budgeting.md)
- [生成AIによるプレゼン資料・ドキュメント作成の実務活用](ai-presentation-and-document-creation.md)
- [生成AIによる情報収集・リサーチの実務活用(Deep Research機能)](ai-research-and-information-gathering.md)
- [生成AI導入の社内展開・浸透のすすめ方](ai-adoption-rollout-basics.md)
- [生成AIによる画像・ビジュアル作成の実務活用(資料・広告・SNS用画像)](ai-image-and-visual-creation-for-business.md)

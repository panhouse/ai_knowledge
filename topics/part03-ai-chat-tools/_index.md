---
title: "Part 3: AIチャットツールの基本"
part: 3
---

# Part 3: AIチャットツールの基本

ChatGPT / Gemini / Claude / Copilot などの汎用AIチャットツールを、
特定ツールを本流とせず**横断比較**で扱う。機能の概念は章で束ね、
ツール固有の詳細は「◯◯の基本」ページ(第5章)に分離する。

## 章構成(分類の地図)

### 第1章 プラン・モデルの選び方
- 主要ツールのプラン比較、モデル一覧、料金対効果の判断

### 第2章 初期設定とデータ保護
- 初期設定、データ利用のオプトアウト、カスタム指示(各ツール対応表)

### 第3章 記憶・文脈の管理
- メモリ、プロジェクト系機能、会話の使い分け(各ツール対応表)

### 第4章 生成・分析の主要機能
- データ分析、Canvas、Web検索、画像生成、Agent/Tasks、音声

### 第5章 主要ツール各論
- ChatGPT、Gemini、Claude、Copilot、DeepSeek、Grok の個別ガイド

## 収録ページ

- [主要AIチャットツールのプラン・料金・モデル横断比較(ChatGPT/Gemini/Claude/Copilot)](ai-chat-tools-plan-model-comparison.md)
- [ChatGPTのプラン比較](chatgpt-plan-comparison.md)
- [ChatGPTのモデル一覧と使い分け](chatgpt-model-lineup.md)
- [ChatGPTの初期設定とデータ利用のオプトアウト](chatgpt-initial-setup-and-opt-out.md)
- [ChatGPTの初期設定とカスタム指示の書き方](chatgpt-custom-instructions.md)
- [Gemini・Claude・Copilotの初期設定とデータ利用オプトアウト比較](ai-chat-tools-privacy-and-setup-comparison.md)
- [ChatGPTのメモリ(Memory)機能](chatgpt-memory-feature.md)
- [ChatGPTの「プロジェクト」機能](chatgpt-projects-feature.md)
- [主要AIチャットツールのメモリ・プロジェクト機能比較(Gemini・Claude・Copilot)](ai-chat-tools-memory-and-context-comparison.md)
- [ChatGPTのCanvas機能](chatgpt-canvas-feature.md)
- [ChatGPTのWeb検索機能](chatgpt-web-search-feature.md)
- [ChatGPTの画像生成機能](chatgpt-image-generation-feature.md)
- [主要AIチャットツールのデータ分析機能比較(ChatGPT・Gemini・Claude・Copilot)](ai-chat-tools-data-analysis-comparison.md)
- [ChatGPTのエージェント機能(ChatGPT Agent)とスケジュールタスク(Tasks)](chatgpt-agent-mode-feature.md)
- [Gemini Spark(Google)の基本](gemini-spark-basics.md)
- [ChatGPTの音声(Advanced Voice Mode)機能](chatgpt-voice-mode-feature.md)
- [主要AIチャットツールのエージェント機能・スケジュールタスク比較(ChatGPT Agent/Tasks・Gemini Spark・Copilot Cowork・Claude Cowork)](ai-chat-tools-agent-tasks-comparison.md)
- [主要AIチャットツールのCanvas/Artifacts機能比較(ChatGPT・Gemini・Claude・Copilot)](ai-chat-tools-canvas-artifacts-comparison.md)
- [主要AIチャットツールの画像生成機能比較(ChatGPT・Gemini・Claude・Copilot)](ai-chat-tools-image-generation-comparison.md)
- [主要AIチャットツールの音声対話機能比較(Advanced Voice Mode・Gemini Live等)](ai-chat-tools-voice-comparison.md)
- [主要AIチャットツールのWeb検索機能比較(ChatGPT・Gemini・Claude・Copilot)](ai-chat-tools-web-search-comparison.md)
- [Google Geminiの基本](google-gemini-basics.md)
- [Claude(Anthropic)の基本](claude-basics.md)
- [Microsoft Copilotの基本](microsoft-copilot-basics.md)
- [DeepSeekの基本](deepseek-basics.md)
- [Grok(xAI)の基本](grok-basics.md)
- [Meta AIの基本](meta-ai-basics.md)

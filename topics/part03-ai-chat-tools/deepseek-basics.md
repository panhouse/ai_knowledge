...
かつて「推論モデル」として話題を集めたR1系は、V4では独立した別モデルではなく「思考モード」(Non-Think/Think High/Think Maxの3段階)としてV4-Pro・V4-Flashに統合されている。V4は2026年4月24日にプレビュー版として先行公開された後、2026年7月17日に正式版(GA)へ昇格した。モデルの構成(パラメータ規模・コンテキスト長)はプレビュー版から変更されていないが、正式版移行にあわせてピーク/オフピークの2段階課金が導入され、さらに2026年8月16日16:00(UTC)にはその単価自体が引き上げられている(詳細は後述の「実務での使い方」を参照)。

次世代モデル「R2」は2026年8月時点でも公式発表・提供開始に至っていない。2025年5月・同年8月・2026年5月と、これまでに少なくとも3回の延期が報じられている(2025年8月には「同月15〜30日に発表される」との市場の憶測が浮上しDeepSeek関係者が否定した経緯もある)。発表観測が浮かんでは否定される、というパターンをその後も繰り返しており、海外メディアの報道によれば、中国当局の意向で学習基盤を米国製(NVIDIA)から中国製(Huawei Ascend)へ切り替えようとした結果、学習クラスタの不安定さやソフトウェア基盤の未成熟によって学習がうまく進まず、最終的に学習はNVIDIA製、推論はHuawei製という体制に落ち着いたと伝えられている。加えてCEOの梁文峰(Liang Wenfeng)氏がR2の性能に満足していないとの報道もあり、リリース時期は2026年8月時点でも未定の状態が続いている。
...
料金・性能は数か月単位で変動するため、実際の見積もりの際は各社公式のPricingページで最新値を確認すること。特にDeepSeekは2026年8月16日にピーク/オフピークの単価そのものを引き上げており(後述)、他社より変動要因が1つ多い。値上げ後も依然としてChatGPT・Claude・Geminiの標準モデルより大幅に安いが、以前ほどの価格差ではなくなっている点には留意する。
...
- 主なモデルID: `deepseek-v4-pro`(高性能)、`deepseek-v4-flash`(軽量・低コスト)。旧モデル名`deepseek-chat`/`deepseek-reasoner`は2026年7月24日15:59(UTC)に廃止済みで、以降はAPIエラーとなる。該当コードが残っている場合はモデル名を書き換えるだけで移行できる
- **DeepSeekは2026年7月17日のV4正式版(GA)移行時にピーク/オフピークの2段階課金を導入し、続く2026年8月16日16:00(UTC)にはその単価自体を引き上げた**。ピーク時間帯は01:00〜04:00と06:00〜10:00(UTC、北京時間では9:00〜12:00・14:00〜18:00に相当)で、オフピーク料金はピーク料金のちょうど半額という構造そのものは変わっていない。DeepSeekは公式チェンジログで「V4モデルファミリーの正式リリースに伴い、資源をより合理的に配分するため」料金を改定すると説明しており、利用の平準化を狙った措置とみられる
...
- 契約・見積もり前には必ず[DeepSeek公式のPricingページ](https://api-docs.deepseek.com/quick_start/pricing)で最新の金額を確認すること(2026年8月だけで2度目の料金改定であり、変動が速い)

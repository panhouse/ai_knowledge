...
一方で、2026年9月時点でもGPT-Liveはまだ**カメラ映像・画面共有に対応していない**。資料を見せながら質問したり、カメラを向けて「これは何?」と尋ねたりする使い方をしたい場合は、音声セッション開始時に旧来の「Advanced Voice Mode(レガシー)」を選択する必要がある。OpenAIは7月の発表時から「対応予定」としているが、9月時点でも具体的な提供時期は明らかにされていない。

もう一つの変化が、生成音声への**電子透かし(SynthID)**の付与である。2026年7月31日以降、GPT-LiveおよびAudio APIで生成された音声にはSynthIDの電子透かしが埋め込まれるようになり、OpenAIが公開する判定ツールでAI生成音声かどうかを確認できるようになった(ただし「透かしが検出されない=人間の声」という保証にはならない点に注意)。
...
- **スマートフォン(iOS/Android)**: ChatGPTアプリでチャット画面を開き、入力欄右下の波形アイコンをタップすると音声セッションが始まる。既定エンジンは新しい「GPT-Live」
- **パソコン(Web版・Windows/Mac用デスクトップアプリ)**: チャット入力欄にあるヘッドフォン(またはマイク)のアイコンをクリックすると音声セッションが始まる。2026年7月23日以降Mac用デスクトップアプリに、その後Windows版にもGPT-Liveが展開され、手前に開いているウィンドウの内容やファイルアップロード・プロジェクトの中身を参照しながら話せるようになった
...
| Claude(Anthropic) | ネイティブな音声会話機能は2026年9月時点で未提供 | モバイルアプリの音声入力は「音声→テキスト変換」止まりで、GPT-Liveのような音声対音声・全二重のやり取りはできない |

### 料金・利用制限の目安(2026年9月時点)

| プラン | 使えるモデル | 音声モードの利用制限の目安 |
|---|---|---|
| Free($0) | GPT-Live-1 mini(軽量版) | 直近24時間のローリングウィンドウで利用量が限られ、上限に達するとアプリ内で通知される(具体的な分数は非公開・変更されやすい) |
| Go($8/月) | GPT-Live-1(フル版) | Freeよりまとまった時間を利用可能 |
| Plus($20/月) | GPT-Live-1(フル版) | 1日数時間程度まで利用可能な目安 |
| Pro($100/月・$200/月の2段階) | GPT-Live-1(フル版)。カメラ・画面共有が必要な場合は旧Advanced Voice Modeも選択可 | 不正利用防止のガードレールの範囲内でほぼ無制限。2026年4月に新設された$100の下位Pro枠と、従来からの$200枠が併存しており、両者の差はCodex(コーディング)利用量など他機能が中心で、音声モード自体の上限差は小さい |
| Business(旧Team)・Enterprise・Edu | 組織のプラン・管理者設定に準じる | 組織のプラン・管理者設定に準じる |
...
- **カメラ・画面共有は既定エンジンでは使えない**: 2026年9月時点のGPT-Liveは音声のみに対応で、資料を見せながら質問する用途には非対応。この用途では音声セッション開始時に旧来の「Advanced Voice Mode(レガシー)」を明示的に選ぶ必要がある。OpenAIは対応を予告しているが、9月時点でも具体的な提供時期は未定のまま
- **同時通訳は完璧ではない**: リアルタイム翻訳は便利で、実際に使ってみたレビューでも訳出の速さ・自然さは高く評価されているが、固有名詞・専門用語・数字の訳し間違いが起こりうる。契約交渉など訳語の正確性が重要な場面では、後でテキストログを見返して確認する運用にする
- **生成音声にはSynthIDの電子透かしが入る**: 2026年7月31日以降、GPT-LiveやAudio APIで生成した音声にはSynthIDの電子透かしが埋め込まれ、OpenAIの公開判定ツールでAI生成かどうかを確認できるようになった。ただし透かしが検出されないからといって「人間の声」と断定はできない(メタデータの除去や劣化により検出できない場合がある)
...
### 2026-09-13: 料金体系・電子透かしなど2026年9月時点の状況に更新
- **内容**: カメラ・画面共有はGPT-Liveでも2026年9月時点でまだ非対応であることを再確認して記述を更新。2026年4月に新設された$100のPro枠(従来の$200枠と併存、差はCodex利用量が中心)を料金表に反映。「Team」プランが「Business」に改称された点を反映。2026年7月31日からGPT-Live/Audio APIの生成音声にSynthID電子透かしが付与されるようになった点を注意点に追加。Windows版デスクトップアプリへのGPT-Live展開、ファイルアップロード・プロジェクト連携対応を追記
- **出典**: [GPT-Live vs Advanced Voice Mode: What Changed | Apidog](https://apidog.com/blog/gpt-live-vs-advanced-voice-mode/)、[GPT-Liveとは｜チャットGPTライブの使い方・料金【2026年9月】 | Uravation](https://uravation.com/media/gpt-live-openai-guide-2026/)、[ChatGPTの音声モードの違い｜GPT-Live・Advanced・Standardの使い分け | keieiax](https://keieiax.jp/blog/chatgpt-gpt-live/)、[OpenAI Adds SynthID Watermarking to GPT-Live Voice Audio | Globe Market Research](https://www.globemarketresearch.com/industry-news/openai-adds-synthid-watermarking)、[ChatGPT finally offers $100/month Pro plan | TechCrunch](https://techcrunch.com/2026/04/09/chatgpt-pro-plan-100-month-codex/)、[ChatGPT Business Pricing 2026 | Justin McKelvey](https://justinmckelvey.com/blog/chatgpt-business-pricing)

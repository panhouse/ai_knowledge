...
| 社外向けサービスや、同時に多数のユーザーが使うシステムに組み込みたい | △ 不向き(後述の実行時間・クォータ制限があり本番運用には力不足) | 自社サーバー、[Dify](../part09-nocode-lowcode/dify-workflow-nodes.md)などのノーコードAIアプリ基盤 |
...
- **本番の社外向けシステムには使わない**: GASは可用性やスケーラビリティを保証する仕組みではないため、多数のユーザーが同時にアクセスするサービスや、ミスがあった際の影響が大きい業務には不向き。その場合は自社サーバーや[Dify](../part09-nocode-lowcode/dify-workflow-nodes.md)などの専用基盤、大量データなら[バッチ処理(Batch API)の基本](batch-api-basics.md)を検討する。

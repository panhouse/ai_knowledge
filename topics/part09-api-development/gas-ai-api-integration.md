...
Google自身のGemini APIについては、Google Cloudの公式Codelab(「Automate Google Workspace tasks with the Gemini API」)がGASからの呼び出し方法を解説しており、この組み合わせが公式にドキュメント化されている点はOpenAI・Anthropicとの違いといえる。手軽に試すだけなら、この記事のOpenAI・Claudeと同じ「APIキー+UrlFetchApp」方式で十分だが、Geminiにはもう一段"native"な選択肢もある。GASのスクリプトエディタには**Vertex AI**という組み込みのAdvanced Service(GmailApp・DriveAppと同じ位置づけの、コードを書かずに有効化できる専用サービス)があり、左メニューの「サービス」の「+」→「Vertex AI API」を追加すると、UrlFetchAppを自分で書かなくても`VertexAI.Endpoints.generateContent(payload, model)`のような専用メソッドでGeminiを呼び出せる。ただしこちらは個人のAPIキーではなく、課金を有効化したGoogle CloudプロジェクトとAgent Platform API(旧Vertex AI API)の有効化が前提になるため、セットアップはAI StudioのAPIキー方式より重い。公式のQuickstartが例示に使うモデルは執筆時点でGemini 2.5 Flashだが、Advanced Serviceのモデル自動検出はプレビュー版モデルや一部リージョンで内部エラーが出ることがあるため、Gemini 3系のような新しい世代へ切り替える際は正式リリース(GA)版のモデルIDを使う方が無難。**個人・小規模チームでまず試すならAPIキー+UrlFetchApp、社内で既にGoogle Cloudプロジェクトを運用しているならVertex AI Advanced Serviceの方がAPIキーの受け渡し・失効管理が要らず安全**、という使い分けになる。本記事では汎用性の高いAPIキー+UrlFetchApp方式を中心に解説する。
...
- **出典**: [Google for Developers: Quotas for Google Services](https://developers.google.com/apps-script/guides/services/quotas)、[AppScriptExpert: Google Apps Script Quotas & Limits](https://appscriptexpert.com/blog/google-apps-script-quotas-and-limits)、[Google for Developers: Gemini 3.6 Flash](https://ai.google.dev/gemini-api/docs/models/gemini-3.6-flash)、[Google for Developers: Using the latest Gemini models](https://ai.google.dev/gemini-api/docs/latest-model)、[Google for Developers: Quickstart: Generate text using Agent Platform](https://developers.google.com/apps-script/quickstart/vertex-ai)、[justin.poehnelt.com: Using Gemini in Apps Script](https://justin.poehnelt.com/posts/using-gemini-in-apps-script/)、[OpenAI: GPT-5.6](https://openai.com/index/gpt-5-6/)、[Claude Platform Docs: Models overview](https://docs.claude.com/en/docs/about-claude/models/overview)

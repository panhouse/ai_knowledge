...
part: 9
chapter: 第2章 API活用実践
tags: [Batch API, API, コスト削減, 非同期処理, JSONL, RAG, Mistral]
created: 2026-07-06
updated: 2026-09-09
---
...
「サポートチケット3万件をAIで一括分類したい」「社内文書アーカイブを全部要約したい」といった大量処理を、通常のAPI(1件ずつ呼び出す方式)でやろうとすると、レート制限(一定時間あたりのリクエスト数上限)に引っかかったり、料金が想定以上にかさんだりする。Batch API(バッチAPI)は、大量のリクエストを1つのファイルにまとめて送信し、「即答は不要なので、24時間以内など決まった猶予の中でまとめて処理してほしい」という形で依頼する仕組みで、通常の半額程度の料金で処理できる。OpenAI・Anthropic・Googleに加え、2026年5月にはMistral AIもBatch APIを追加するなど提供元は広がっている(2026年9月時点)。ただし「バッチ=一律50%割引」とは限らず、xAI(Grok)のように旧世代モデルのみ20%割引・最新の主力モデルは割引なしとするプロバイダーもあるため、乗り換えや新規プロバイダー採用時は必ず各社の最新の割引率を確認する。
...
| 料金 | 定価 | 約50%割引(OpenAI・Anthropic・Google・Mistralともに。ただし全プロバイダー共通ではない) |
...
2026年9月時点でOpenAIのBatch APIが対応する主なエンドポイントは`/v1/chat/completions`(チャット形式の生成)・`/v1/responses`・`/v1/completions`・`/v1/embeddings`(埋め込み生成)・`/v1/moderations`・`/v1/images/generations`・`/v1/images/edits`・`/v1/videos`など、画像・動画生成系にも対応範囲が広がっている。1つのバッチファイルには最大5万件のリクエスト、ファイルサイズは最大200MBまでという制限があり、`/v1/embeddings`は1バッチあたり最大5万件の埋め込み入力までという別枠の上限に加えて、組織全体でのキュー投入待ちリクエスト数が最大100万件までという枠も持つ。さらにバッチの「作成」自体にも1時間あたり最大2,000バッチまでというレート制限があるため、大量データを少数の大きなバッチにまとめる設計が向いている(件数・容量の上限は変更されることがあるため、必ず公式ドキュメントで確認する)。料金はどのモデルでも一律で通常価格の約50%になる。結果ファイルはOpenAI側で自動的には削除されず、明示的に削除するか、アップロード時に`expires_after`(14〜30日で設定可)を指定しない限り残り続ける点はAnthropic・Googleの「一定日数で自動的にダウンロード不可になる」方式と異なる。
...
1件のバッチは最大10万リクエストまたは256MBのいずれか早い方が上限。処理は`processing_status`が`in_progress`→`ended`に変わるのをポーリングで確認し、`ended`になったら`results_url`からJSONL形式の結果を取得する。結果は成功(`succeeded`)・エラー(`errored`)・キャンセル(`canceled`)・期限切れ(`expired`)の4種類のいずれかで返り、エラー・キャンセル・期限切れの分は課金されない。料金はどのモデルでも入力・出力ともに標準価格の50%で、プロンプトキャッシュ(直前と同じ内容の再利用を割引く仕組み)の割引とも併用できる。全アクティブモデルがBatch APIに対応しており、画像入力(Vision)・Tool Use(Web検索・コード実行・MCPコネクタ・Advisor・Tool Searchなどのサーバー側ツールを含む)・拡張思考(Extended Thinking)もそのまま使える。バッチは処理に数分〜1時間以上かかることがあるため、プロンプトキャッシュを使う場合は通常の5分キャッシュではなく1時間キャッシュ(TTL指定)にしておくとキャッシュヒット率が上がる。処理結果は作成から29日間ダウンロード可能で、それを過ぎるとバッチ自体は参照できても結果ファイルは取得できなくなる。なお2026年9月時点、Claude Opus 5・Opus 4.8〜4.6・Sonnet 5・Sonnet 4.6では、ベータヘッダー`output-300k-2026-03-24`を指定するとバッチ限定で`max_tokens`の上限が通常の128kから300kトークンまで拡張できる(書籍規模の長文生成や大量の構造化データ抽出向け。同期のMessages APIやAmazon Bedrock/Google Cloud/Microsoft Foundry上のClaudeでは非対応)。
...
2026年9月時点で、Gemini Batch APIはテキスト生成(`generateContent`)に加えて埋め込みモデル(`gemini-embedding-001`など)のバッチ処理にも対応し、さらにOpenAI SDKと同じ書き方(APIキーとベースURLをGoogle側に向けるだけ)でバッチを作成・状態確認・結果取得できる「OpenAI互換レイヤー」も追加された(ただしOpenAI互換レイヤーでのファイルのアップロード・ダウンロードは非対応で、通常のGemini APIの方式を使う必要がある)。Batch API用のレート制限は通常の同期呼び出しとは別枠で、より高いスループットが確保されている一方、同時に処理できるバッチジョブ数は最大100件・ファイルストレージの合計容量は最大20GBまでという上限がある(モデル・利用ティアによって変動するため公式ドキュメントで要確認)。

### Mistral Batch API の使い方(概念フロー)

2026年5月にMistral AIも自社プラットフォーム「La Plateforme」でBatch APIを追加した(2026年9月時点ではLa Plateforme上の全モデルが対象で、クラウドパートナー経由の提供は順次拡大中)。基本的な流れはOpenAIに近い。

1. リクエストをまとめたJSONLファイルを用意し、Files APIでアップロードする。
2. バッチジョブ作成エンドポイントに、アップロードしたファイルIDと使用するモデル・エンドポイントを指定して送信する。
3. ジョブが完了するまでステータスをポーリングし、完了後に結果ファイルをダウンロードする。

割引率は通常の同期呼び出し比で50%、1ジョブで最大100万件規模のリクエストを非同期処理できるとされ、顧客フィードバック分析・大量文書の要約や翻訳・埋め込み生成・データラベリングといった用途が想定されている。

### 比較表: OpenAI vs Anthropic vs Google Gemini vs Mistral のBatch API(2026年9月時点)

| 項目 | OpenAI Batch API | Anthropic Message Batches API | Google Gemini Batch API(Batch Mode) | Mistral Batch API |
|---|---|---|---|---|
| 割引率 | 約50%(全モデル一律) | 50%(全モデル一律、入力・出力とも) | 約50%(標準の同期呼び出し比) | 約50%(標準の同期呼び出し比) |
| 完了までの目安 | 24時間以内(実際は1〜6時間程度で終わることが多い) | 24時間以内(実際は1時間以内で終わることが多い) | 24時間以内(実際はより短時間で終わることが多い) | 24時間以内が目安 |
| リクエストの渡し方 | JSONLファイルをFiles APIでアップロード | APIリクエストの`requests`配列に直接まとめて送信 | インライン埋め込み(20MB未満目安)、またはJSONLファイルのアップロード | JSONLファイルをFiles APIでアップロード |
| 1バッチの上限 | 最大5万リクエスト、ファイルサイズ200MBまで(埋め込みは組織全体で最大100万件のキュー投入待ち上限も別途あり) | 最大10万リクエスト、256MBまで | ファイルサイズ最大2GBまで、同時実行ジョブ最大100件・ストレージ合計最大20GBまで | 1ジョブ最大100万件規模まで対応 |
| 対応エンドポイント・モデル | `/v1/chat/completions`、`/v1/responses`、`/v1/completions`、`/v1/embeddings`、`/v1/moderations`、画像・動画生成系など | Messages API相当のほぼ全機能(画像入力・Tool Use・システムプロンプト・マルチターン・拡張思考など、全アクティブモデル対応。一部モデルはベータで`max_tokens`300kまで拡張可能) | テキスト生成(`generateContent`)、埋め込み(`gemini-embedding-001`など)、OpenAI SDK互換の`/v1/chat/completions`形式 | La Plateforme上のほぼ全モデル(チャット・埋め込みなど) |
| 結果ファイルの保存期間 | 自動削除なし(手動削除、または`expires_after`で14〜30日を設定可能) | 作成から29日間ダウンロード可能 | 個別に確認(公式ドキュメント参照。ジョブ自体は一定期間後にクリーンアップされる) | 公式ドキュメント参照(本稿執筆時点で日数の明記は未確認) |
| 結果の識別方法 | `custom_id`で突き合わせ(順序は保証されない) | `custom_id`で突き合わせ(順序は保証されない) | リクエストの並び順、またはインライン応答内の対応関係で突き合わせ | `custom_id`相当のIDで突き合わせ |

4社とも「半額前後・24時間以内・JSON系フォーマットでの一括投入・専用のレート制限枠」という基本構造は共通しているため、複数プロバイダーを併用する場合も考え方を使い回しやすい。ただし結果ファイルの保存期間や対応エンドポイントの細部は異なるため、乗り換えや併用時は必ず各社の最新ドキュメントで確認する。

**注意: 「バッチ=50%割引」が業界標準とは限らない。** 2026年9月時点でxAI(Grok)のBatch APIは、旧世代の一部モデル(Grok 4.3、Grok 4.20系など)には約20%の割引を付けるが、最新の主力モデルであるGrok 4.6には割引を適用していない。新しいプロバイダーをBatch API目的で採用する際は、上表の4社のような「一律50%」を前提にせず、各社の料金ページで対象モデルごとの割引率を個別に確認すること。
...
- **Batch用のレート制限は通常のAPIと別枠**: 各社とも「バッチ専用の投入待ちキュー上限」を別に持つため、通常のAPIのレート制限には直接影響しない一方、バッチ自体にも独自の上限(同時実行数・投入待ちリクエスト数など)があるので、大量のバッチを同時に投げすぎると待ち行列が詰まることがある。
- **料金・上限は変更が多い分野**: バッチの割引率・上限件数・対応エンドポイントは各社とも改定が入りやすいため、本番運用前に必ず公式ドキュメントの最新情報を確認する。
- **「バッチなら必ず半額」と思い込まない**: OpenAI・Anthropic・Google・Mistralは2026年9月時点で軒並み約50%割引だが、xAI(Grok)のように最新モデルには割引なし・旧モデルのみ割引という一律ではないプロバイダーも出てきている。新しいプロバイダーを比較検討する際は「Batch APIがある=半額」と早合点せず、対象モデルごとの割引率を必ず個別に確認する。
...
### 2026-09-09: Mistral Batch API・xAI Grokの割引実態を追記し、3社の上限・機能を2026年9月時点に最新化
- **内容**: 2026年5月に追加されたMistral AI Batch API(La Plateforme上の全モデル対象、割引約50%、1ジョブ最大100万件規模)を新規に1セクション追加し、比較表を4社比較に拡張。xAI(Grok)のBatch APIが旧モデルのみ約20%割引・最新の主力モデルGrok 4.6には割引なしという「バッチ=一律50%割引ではない」実態を「使いどころ」「注意点」に追記。OpenAIはバッチ作成自体の1時間あたり2,000件レート制限と埋め込み向け組織全体キュー上限(最大100万件)を追記。Anthropicはサーバー側ツールにAdvisor・Tool Searchが加わったことと、ベータヘッダー`output-300k-2026-03-24`によるバッチ限定の出力上限拡張(300kトークンまで)を追記。Google Gemini Batch APIは同時実行ジョブ数(最大100件)・ストレージ合計上限(最大20GB)を追記
- **出典**: [Anthropic: Batch processing](https://platform.claude.com/docs/en/build-with-claude/batch-processing)、[Mistral AI: Batch API](https://mistral.ai/news/batch-api/)、[xAI: API Pricing](https://docs.x.ai/developers/pricing)、[OpenAI Help Center: Batch API FAQ](https://help.openai.com/en/articles/9197833-batch-api-faq)、[OpenAI Developer Community: OpenAI file size limit and Batch API result files](https://community.openai.com/t/openai-file-size-limit-and-batch-api-result-files/897738)、[Google AI for Developers: Batch API](https://ai.google.dev/gemini-api/docs/batch-api)、[Google AI for Developers: Rate limits](https://ai.google.dev/gemini-api/docs/rate-limits)

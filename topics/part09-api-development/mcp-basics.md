...
AIチャットに「Slackの未読を要約して」「Google Driveのこの資料を読んで」と頼んでも、そのままでは何もできない。ChatGPT・Claude・Geminiなど、AIの種類ごと・接続先(Slack、Notion、社内DB…)ごとに、バラバラな方法で「つなぎ込み」のコードを書く必要があったからだ。MCP(Model Context Protocol、モデルコンテキストプロトコル)は、この「AIと外部のツール・データを接続する方法」を統一するために2024年11月にAnthropicが公開したオープンな規格。2026年9月時点では、ChatGPT・Gemini・Microsoft Copilotを含む主要なAIツールがすべて対応を完了しており、主要SDK(TypeScript/Python/Go/C#等)の月間ダウンロード数は合計で5億回近くに達するなど、事実上の業界標準になっている。よく「AIにとってのUSB-C(どの機器にも挿さる共通端子)」と例えられる。
...
- **認証まわりの強化**: MCPサーバーが正式にOAuth 2.1の「リソースサーバー」と位置づけられ、クライアントが正しい認可サーバーを自動発見できるようRFC 9728(OAuth 2.0 Protected Resource Metadata)への対応が必須化。クライアント側もRFC 8707(Resource Indicators)によって「このトークンはどのMCPサーバー宛てか」を明示することが求められ、悪意あるサーバーが他サーバー宛てのトークンを騙し取る攻撃を防ぐ設計になった。RFC 9207(発行者検証)への対応も追加され、動的クライアント登録(DCR)は正式に非推奨となり、新方式「Client ID Metadata Documents(CIMD)」への移行が推奨されている(後方互換のため当面DCRも動作する。後述の「注意点」も参照)
...
MCPのコアメンテナーは2026年8月22日、次の仕様改定に向けた新しいロードマップを公開した。優先領域として、(1) 時間のかかる処理や更新通知をやり取りする「エージェント的メッセージング」の基本機能(現状は拡張機能である「Tasks」を本体仕様に統合する方向性を含む)、(2) HTTPを前提にした通信方式(トランスポート)の一本化、(3) エージェントの身元確認・エンタープライズ向け認証の強化、(4) 既存プリミティブ(Tools/Resources/Prompts)の改善、(5) 各言語SDKの開発者体験向上、の5つを挙げている。仕様は今後半年〜1年でさらに変わる見込みであることを踏まえ、自社でMCPサーバーを開発・運用する場合は公式ロードマップを定期的に確認しておきたい。

### Enterprise Managed Authorization(EMA): 認証の一元管理

2026-07-28版で正式な拡張機能に昇格したのが「Enterprise Managed Authorization(EMA)」。組織が使っているID基盤(Microsoft Entra ID、Oktaなど)を「唯一の判断者」とし、管理者がグループ・役割・条件付きアクセスなどのポリシーを一度設定しておけば、利用者は最初のログイン時点で許可されたMCPサーバーに自動接続できる(アプリごとに個別のOAuth同意を求められない「ゼロタッチ」の仕組み)。Anthropic・Microsoft・Oktaなどが対応を進めており、Microsoftはこの仕組みを使って、Microsoft 365管理センターの「Copilotコネクタ」→「接続」タブからFederated Copilot Connectors(MCPベースの外部データ連携。後述)を一元的に管理・展開できるようにしている。
...
### 主なMCPサーバー・クライアントの例(2026年9月時点)

- **クライアント(MCP対応のAIアプリ)**:
  - **Claude Desktop / Claude Code**: Anthropic製。コネクタ(Connectors)という呼び方でMCPサーバーを追加する
  - **ChatGPT**: 2025年9月にMCP対応(当初は「コネクタ」と呼称)。2025年12月17日付けで呼び方が「コネクタ」から「アプリ(Apps)」に変更され、開発者はOpenAIの「Apps SDK」(MCPをベースに、チャット内に表示するUIまで含めて開発できる仕組み)で作成する。2026年2月22日には、SlackやFigmaの画面をチャット内に表示する拡張仕様「MCP Apps」への完全準拠も公式に案内された。自社独自のMCPサーバーを読み書き両方の権限で接続する「Developer mode(開発者モード)」はPro・Plus・Business・Enterprise・Educationプランで利用でき、Freeプランは対象外(設定手順は後述)
  ...
  - **Microsoft Copilot**: MCPベースの外部データ連携「Federated Copilot Connectors」が2026年9月に一般提供(GA)へ移行し、Microsoft 365 Copilot Chat・Copilot in Excel・Researcherエージェント・Cowork(Microsoftのエージェント型ワークスペース)の各体験で利用できるようになった。管理者はMicrosoft 365管理センターの「Copilotコネクタ」→「接続」から有効化・ロールアウト範囲を管理し、Notion・HubSpot・Box・FactSet・PubMedなど会計/法務/ヘルスケア/開発者ツールなど幅広いカテゴリのコネクタがギャラリーに並ぶ。データは同期・保存されず、利用者本人のID・権限でその都度リアルタイムに取得する設計。2026年半ばには前述のEnterprise Managed Authorization(EMA)にも対応し、利用者ごとのOAuth同意なしに許可済みMCPサーバーへ自動接続できるようになった
...
- **公式MCPレジストリ**: 2025年9月に公開された公式レジストリ(`registry.modelcontextprotocol.io`)は登録が急増しており、独立した集計(2026年9月14日時点)によると、バージョン違いを含む登録エントリの総数は101,219件に達した一方、最新版のみに絞った実質的なサーバー数は31,309件(2026年5月時点の約9,650件から4か月弱で3倍以上に増加)。新規サーバーの登録ペースも2025年9月の月456件から2026年8月には月6,673件へと1年足らずで約15倍に拡大した。ただし同集計では登録サーバーの2割超がソースコードの参照先すら公開していない「レビュー不能」な状態にあるとも指摘されており、件数の伸び=信頼できるサーバーの増加とは限らない点に注意したい。件数は集計方法によって大きく変わるため、正確な数字は都度公式レジストリで確認するのが確実
- Claudeの「Connectors Directory(コネクタ ディレクトリ)」の掲載件数は集計方法によって幅があり(2026年6月時点でAnthropicの公式カテゴリ集計は約30カテゴリ・439件)、週単位で増減するため正確な件数は都度公式ページで確認するのが確実。2026年1月からは、Figma・Slackなど外部サービスのUIプレビューやインタラクティブ要素をチャット内に直接表示できる拡張仕様「MCP Apps」への対応が始まっており、2026-07-28版の仕様改定で正式な拡張機能(Extension)として位置づけが確定した
...
ChatGPTで自社独自のMCPサーバーにつなぐ場合は、「設定→セキュリティとログイン」からDeveloper mode(開発者モード)を有効にしたうえで、「ChatGPTプラグイン」画面の「+」ボタンからMCPサーバーのURLを登録する。Developer modeは読み取り・書き込み双方の操作を許可する強力な機能であるため、プロンプトインジェクションや誤操作によるデータ破壊のリスクを踏まえ、業務利用では信頼できるサーバーに絞って使うことが公式にも推奨されている。

**呼び方の対応(ツール横断)**: 同じ「MCP経由の外部連携」でも、ツールによって画面上の呼び方が異なる。Claude=「コネクタ(Connectors)」/ ChatGPT=「アプリ(Apps)」(2025年12月に「コネクタ」から改称。自社サーバーは「Developer mode」経由)/ Microsoft Copilot=「Federated Copilot Connectors」(M365管理センターで管理者が設定)/ Gemini Enterprise=「リモートMCPサーバー」。画面上でこれらの言葉を見かけたら、いずれもMCPによる外部接続機能だと読み替えるとよい。

### 料金・コストの考え方

MCP自体の利用に追加のライセンス料はかからないオープン規格だが、実際にはつなぐ先のサービス(Slack・GitHub等)の契約プランや、AIアプリ側のプランによって使える範囲が変わる。たとえばChatGPTでは、既製の「アプリ(Apps)」は多くのプランで使えるが、自社独自のMCPサーバーを読み書き両方の権限でつなぐDeveloper modeはPro・Plus・Business・Enterprise・Educationプランに限られ、Freeプランでは使えない。Claudeのコネクタ機能も有料プラン以上が対象になるケースが多い。また、AIエージェントがMCP経由で何度もツールを呼び出す使い方をすると、その分APIのトークン消費・利用量が増える点も踏まえておく。
...
- **進化が非常に速い分野**: 2024年11月の登場から2年弱でOpenAI・Google・Microsoftなど主要ベンダーが相次いで対応を完了し、2025年12月にはガバナンスもAgentic AI Foundationへ移管された。2026年7月28日には「発足以来最大級」とされる仕様改定(プロトコルのステートレス化、Extensions framework、認証強化、非推奨ポリシーの明文化)が正式公開され、8月22日には次の改定に向けた新ロードマップも公開されている。Microsoft CopilotのFederated Copilot Connectorsが2026年9月にGAへ移行するなど、大手ベンダーの対応も「試験提供」から「本番運用」の段階に進みつつある。仕様・対応状況・利用可能なコネクタの一覧は数か月単位で更新されているため、導入判断の前には必ず公式情報で最新状況を確認する
...
### 2026-09-19: MCPレジストリの急拡大・Microsoft Copilotの本番移行(GA)・ChatGPT Developer modeのプラン条件を反映して最新化
- **内容**: 公式MCPレジストリの独立集計(2026年9月14日時点)を追記し、バージョン違いを含む登録エントリが101,219件、最新版のみのサーバー数が31,309件(2026年5月の約9,650件から4か月弱で3倍超に急増)まで拡大したこと、新規登録ペースが1年で約15倍(月456件→月6,673件)に伸びたこと、登録サーバーの2割超がソースコードを公開していない点を追記し、件数の伸びが必ずしも信頼性の向上を意味しない旨を「注意点」に補足。Microsoft CopilotのFederated Copilot Connectorsが2026年9月に一般提供(GA)へ移行し、Microsoft 365 Copilot Chat・Copilot in Excel・Researcherエージェント・Coworkの各体験で使えるようになったことを反映し、対応カテゴリの例を追記。ChatGPTの「Developer mode」(自社独自MCPサーバーへの読み書き接続)がPro・Plus・Business・Enterprise・Educationプランで利用可能でFreeプランは対象外であること、設定手順(設定→セキュリティとログイン→ChatGPTプラグイン)を「実務での使い方」「料金・コストの考え方」に追記。2026年8月22日に公開されたMCPの新ロードマップ(エージェント的メッセージング・HTTPトランスポート統合・エージェント認証・プリミティブ改善・SDK DXの5領域)を新規の段落で紹介
- **出典**: [DEV Community: I counted the MCP registry: 101,219 entries are 31,309 servers, and 23% have no source repo](https://dev.to/leroy_jenkins_951c84b2838/i-counted-the-mcp-registry-101219-entries-are-31309-servers-and-23-have-no-source-repo-33a6), [Microsoft Learn: Federated connectors overview - Microsoft 365 Copilot connectors](https://learn.microsoft.com/en-us/microsoft-365/copilot/connectors/federated-connectors-overview), [Big Hat Group: Copilot Weekly - Grok Joins M365, MCP Connectors Hit GA (2026-09-14)](https://www.bighatgroup.com/blog/copilot-weekly-2026-09-14/), [OpenAI Developers: Developer mode](https://developers.openai.com/api/docs/guides/developer-mode), [Model Context Protocol Blog: The New MCP Roadmap](https://blog.modelcontextprotocol.io/posts/mcp-roadmap/)

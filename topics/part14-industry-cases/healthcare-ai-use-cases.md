...
| 患者向け説明・問い合わせ対応 | 診察時間の制約による説明不足、患者の不安・疑問への対応不足 | AIアバターやチャットボットが問診・症状の聞き取りや、疾患・治療の説明を行う | 大阪国際がんセンター「対話型疾患説明生成AI」/ OpenAI「ChatGPT Health」 |
...
- **効果**: Ubie生成AIについては、南部徳洲会病院がIC記録作成など月間約4,000件の
  ...
  2026年7月に発表された九州がんセンターの事例では、初診時のカルテ入力時間を
  約10分短縮し、カンファレンス等の議事録作成にかかる時間を70%削減したと報告されており、
  「カルテ記載」に加えて会議の記録業務にも活用範囲が広がっていることがわかる
...
- **導入したAI・仕組み**: 国内初とうたうオンプレミス(自院サーバー内で完結させる構成)と
  ...
  医療法人大誠会内田病院に導入された。2026年3月にはハイブリッドAI構成を強化した
  ver.3.0、同7月22日には申し送り業務(勤務交代時の引き継ぎ)を支援するAI機能を
  強化したver.3.1を提供開始。さらに2026年8月10日には、電子カルテに蓄積された
  診療記録・検査結果・処方歴などのテキストデータを横断的に分析し、患者ごとの
  診療経過や重要事項を整理した「病歴要約」のドラフトを自動生成する新機能を追加し、
  社会医療法人祐愛会織田病院で先行導入された
...
- **自社への応用ヒント**: 生成AI活用は大学病院など先進的な大規模施設だけの
  ...
  2025年1月の提供開始から約1年半で「退院時サマリー→意見書→申し送り→病歴要約」と
  対応文書を段階的に広げてきた展開順序は、自院でAIツールを導入する際に
  「どの文書から着手し、次に何へ広げるか」のロードマップとして参考にできる
...
### 4'. 患者向け説明・健康管理(海外発・国内展開中): OpenAI「ChatGPT Health」

- **企業**: OpenAI社
- **課題**: 患者が自分の医療記録・健康データを横断的に把握できず、
  受診前に何を医師へ質問すべきか整理できないまま診察を迎えることが多い
- **導入したAI・仕組み**: 2026年1月7日、通常のチャット履歴と分離した専用空間で
  健康関連の相談に応じる「ChatGPT Health」を発表。米国では電子カルテ連携基盤
  b.wellを介した医療記録の接続に対応し、日本を含む海外ではApple HealthKit
  (歩数・睡眠・心拍数など)や食事記録アプリ「MyFitnessPal」との連携、検査結果PDFの
  アップロードによる質問には対応する一方、医療機関の電子カルテとの連携自体は
  2026年8月時点で米国限定であり、日本の病院データはまだ接続できない。
  健康相談専用のデータには追加の暗号化・分離保護を施し、希望すれば30日以内に
  完全削除できる設計とされる
- **効果**: OpenAIは「健康・ウェルネスに関する質問が全世界で週2億3,000万件以上
  寄せられている」実績を踏まえた機能拡張と説明しており、既存の汎用チャットの
  利用実態から専用機能へ発展させた形である
- **自社への応用ヒント**: 「医療機関が導入する専用ツール」と「患者が個人的に使う
  汎用AIの健康機能」が並走する構図になりつつある。医療機関側は、患者が
  ChatGPT等で得た情報を持って来院することを前提に、診察時にその内容を
  確認・補正する対話設計(トリアージや問診票への組み込みなど)を
  検討する価値がある。また日本では電子カルテ連携が未対応である点は、
  自院の情報システム部門が「海外の患者向けAIサービスとどこまで連携できるか」を
  判断する際の現状把握として押さえておきたい
...
- **効果**: 特発性肺線維症(IPF)治療薬候補「レントセルチブ(Rentosertib、旧称
  ...
  さらに2026年8月1日には、フロンティアAI・基盤モデルが創薬の実務課題
  (医薬品化学・化学合成・疾患生物学・臨床開発・老化研究)をどこまで解けるかを
  評価する業界初の標準ベンチマーク「DDD Benchmarks as a Service」を発表した。
  既存の評価用データが学習データに紛れ込みやすいという問題意識から、
  実務課題への対応力を測る仕組みとして提供する
- **自社への応用ヒント**: 医薬品開発における生成AIの価値は「候補の質」ではなく
  ...
  通過し得ることを示す材料として参考になる。また「DDD Benchmarks」が示すように、
  生成AIの性能評価を「ベンチマークのスコア」だけで鵜呑みにせず、
...
- **個人情報・医療データの取り扱い**: 患者の会話・カルテ情報を生成AIに
  ...
  資するための匿名加工医療情報・仮名加工医療情報に関する法律)の枠組みを
  踏まえる必要がある。実務では「閉域接続」「入力データを学習に使わない設定」を
  必須条件とし、まずは議事録要約など患者情報を含まない領域から導入し、
  段階的に診療記録作成・臨床支援へと広げるアプローチが取られることが多い。
  なお2026年7月10日に成立・同17日に公布された改正個人情報保護法
  (令和8年法律第56号)では、AIモデルの開発・データ利活用を目的とする場合に限り、
  第三者への個人データ提供に本人同意を不要とする特例が新設され、
  病歴などの要配慮個人情報もこの特例の対象に含まれる(施行は公布から2年以内)。
  AI開発を後押しする一方、全国保険医団体連合会など医療関係団体からは
  「機微性の高い病歴情報が本人同意なく外部提供され得る」ことへの懸念も
  表明されており、2026年8月時点では詳細な要件(委員会規則で定める開示事項・
  利用目的の限定など)を含め議論が続いている。医療機関がこの特例を根拠に
  独自判断で患者データを外部提供してよいことにはならず、
  次世代医療基盤法など医療分野固有の枠組みとは別トラックの制度である点を
  混同しないよう注意が必要である
...
- **内容**: 患者向け説明・問い合わせ対応の項目にOpenAI「ChatGPT Health」
  (2026年1月発表、日本では電子カルテ連携は米国限定でApple Health連携等にとどまる点を含む)を
  新事例として追加。OPTiM AIホスピタルの節をver.3.1(申し送り業務支援、2026年7月)・
  病歴要約自動生成機能(2026年8月10日、織田病院で先行導入)まで更新し、
  「文書対応範囲を段階的に広げる」展開順序をヒントとして追記。Ubie生成AIの節に
  九州がんセンターの事例(初診カルテ入力10分短縮・議事録70%削減)を追加。
  Insilico Medicineの節に創薬AI評価基盤「DDD Benchmarks as a Service」
  (2026年8月1日発表)を追加。注意点の「個人情報・医療データの取り扱い」を、
  2026年7月成立の改正個人情報保護法(令和8年法律第56号、AI開発目的の
  第三者提供に本人同意不要の特例・要配慮個人情報である病歴も対象、
  医療関係団体からの懸念表明を含む)を踏まえて全面的に書き直した
- **出典**:
  [Introducing ChatGPT Health(OpenAI公式)](https://openai.com/index/introducing-chatgpt-health/)、
  [OpenAI、健康やヘルスケアの情報を一元化する「ChatGPT Health」(マイナビニュース)](https://news.mynavi.jp/techplus/article/20260108-3942019/)、
  [ChatGPT Healthとは？使い方・連携アプリ・日本での利用・プライバシーを完全解説【2026年最新】(.AI TIMES)](https://dot-ai.myuuu.co.jp/times/articles/281)、
  [病院向け生成AIサービス「OPTiM AI ホスピタル」ver.3.1、申し送り業務を支援するAI機能を強化(OPTiM)](https://www.optim.co.jp/newsdetail/20260722-pressrelease-01)、
  [病院向け生成AIサービス「OPTiM AI ホスピタル」、病歴要約を自動生成する新機能を提供開始(IT Leaders)](https://it.impress.co.jp/articles/-/29676)、
  [オプティム、「OPTiM AI ホスピタル」に病歴要約の自動生成機能を追加(クラウド Watch)](https://cloud.watch.impress.co.jp/docs/news/2132250.html)、
  [Ubie、厚労省補助事業に対応した「生成AIサービス特別パッケージ」を提供開始(DOC WEB)](https://doctokyo.jp/mnews/ubie-2/)、
  [Insilico Medicine Launches the Industry's First Drug Discovery and Development (DDD) Benchmark as a Service(Insilico Medicine公式)](https://insilico.com/news/hxjtp34a41-insilico-medicine-launches-the-industrys)、
  [Insilico Medicine DDD Benchmarks: Evaluating AI Models for Real-World Drug Discovery Performance(Drug Target Review)](https://www.drugtargetreview.com/insilico-medicine-launches-ai-drug-discovery-benchmarking-platform/2136128.article)、
  [個人情報保護法改正とは？2026年7月公布の令和8年改正で企業対応はこう変わる(いっそう税理士法人)](https://www.issoh.co.jp/column/details/13787/)、
  [改正個人情報保護法が成立、病歴など要配慮個人情報も同意なく提供可能に-AI開発を後押しの一方で懸念の声(セキュリティ対策Lab)](https://rocket-boys.co.jp/security-measures-lab/amended-personal-information-protection-act-2026/)、
  [【声明】病歴など要配慮個人情報の本人同意なき利活用に舵を切る 個人情報保護法「改悪法案」の廃案を求めます(全国保険医団体連合会)](https://hodanren.doc-net.or.jp/info/declaration/2026-06-15/)
...
### 2026-07-06: 初版執筆
- **内容**: Part14(業種別 生成AI活用事例)の医療・ヘルスケアの章として、

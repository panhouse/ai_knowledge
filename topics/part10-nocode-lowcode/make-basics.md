...
単発のAI呼び出しに加えて、2025年4月に自律的にツールを選び複数ステップを判断させられる「Make AI Agents」機能が発表された後、2026年2月2日には作り直された新版「Make AI Agent(New)」がオープンベータで公開されている。従来のシナリオと同じキャンバス上でエージェントを組み立てられる点が刷新の核で、AIがどのツールをどう選んで実行したかを画面上で追いながら組み立て・修正でき、完成したエージェントは複数のワークフロー・チームをまたいで共有できる。AIの頭脳部分にあたる「AIプロバイダー」は、無料プランを含む全プランで使えるMake組み込みのAIプロバイダーと、OpenAI・Anthropic・Google Geminiなど自分のAPIキーで接続する「カスタムAIプロバイダー」の2種類があり、後者は2025年11月6日以降、Coreプランを含むすべての有料プランで選択できる(それ以前はPro以上限定だった)。ただし現時点(2026年9月)でも日本の実務利用の主流は、n8nの「AI Agentノード」のような自律型エージェントよりも、シナリオの1ステップとしてAIモジュールを組み込む使い方であり、Make AI Agent(New)自体もオープンベータ(機能・料金は変更の可能性あり)という位置づけである。
...
比較表([n8nの基本](./n8n-basics.md)の比較表と対応。2026年9月時点の目安。料金・仕様は変更されやすいため各公式サイトで最終確認すること)
...
### 料金プラン(2026年9月時点の目安。最新情報は make.com/pricing で必ず確認)

2025年8月27日付で、Makeは課金単位を「オペレーション(Operations)」から「クレジット(Credits)」に変更した。標準モジュール(AI以外の連携アプリ)は実行1回=1クレジットのままだが、AIモジュールやコード実行(Make内でのJavaScript/Python実行)は機能・モデル・トークン量に応じてクレジット消費が変動する仕組みになっている点が変更点で、この体系は2026年9月時点でも継続している。

2025年11月6日には、プラン・料金の追加調整が行われた。(1)クレジットを追加購入する際の割増料金が、購入方法(手動購入/自動購入/API経由)によらず一律「25%増」に統一された(従来は自動購入・API購入のみ30%増、手動購入は割増なし)。(2)Coreプランの月間クレジット上限が30万クレジットに設定され、これを超える利用は自動的にProプラン相当に切り替わる。Proプランの月間上限は800万クレジットまで引き上げられ、新しいクレジット階層が追加された。(3)従来はProプラン以上限定だった「カスタムAIプロバイダー接続」(OpenAI・Anthropic等を自分のAPIキーで接続する方式)が、Coreプランを含むすべての有料プランで選べるようになった。
...
- **月間クレジットを使い切ると追加購入に割増料金がかかる**: 2025年11月6日以降、クレジットの追加購入は手動・自動・API経由のいずれでも一律25%増しになった(従来は手動購入のみ割増なし)。プラン選定時はクレジット上限に余裕を持たせておくか、追加購入分の割増を織り込んで予算化する
...
### 2026-09-23: 2025年11月のプラン・料金調整を反映して最新化
- **内容**: 2025年11月6日に実施されたMakeのプラン・料金調整を追加。(1)クレジット追加購入の割増料金が購入方法によらず一律25%増に統一されたこと、(2)Coreプランの月間クレジット上限が30万クレジットに設定され、Proプランの上限が800万クレジットへ引き上げられたこと、(3)従来Pro以上限定だった「カスタムAIプロバイダー接続」がCoreプランを含む全有料プランで選べるようになったことを、仕組み・背景と料金プラン・注意点に反映。日付表記を2026年9月時点に更新
- **出典**: [Make Help Center: Adjustments to plans and pricing](https://help.make.com/adjustments-to-plans-and-pricing)

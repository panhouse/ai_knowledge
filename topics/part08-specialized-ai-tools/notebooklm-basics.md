...
2026年9月15日には、モバイルアプリ(Android・iOS)を中心に5つの新機能が発表された。

- **リアルタイム音声対話**: ノートブックに登録したソースについて声で質問すると、その場でソースに基づく回答が音声で返ってくる機能。説明の途中で割り込んで聞き直すこともできる。約100言語に対応し、まずは18歳以上のGoogle AI Ultra契約者から順次提供が始まり、その後Google AI Proなどにも展開予定
- **ボイスレコーダー**: モバイルアプリに録音機能が追加され、講義や外出先での思いつきをその場で録音すると、選択したノートブックに自動保存される
- **学習の概要**: 要約文に図解(インフォグラフィック)・クイズ・フラッシュカードを埋め込んだレポートを生成する機能。短答式・複数選択式・穴埋め式など新しいクイズ形式にも対応
- **短尺の動画概要(Short Video Overviews)の多言語化**: 約60秒に要点をまとめる動画概要が、日本語を含む80以上の言語に対応した(従来は英語中心)
...
Googleは2026年8月28日、Gemini Notebookの利用上限の仕組みを改定すると発表し、9月2日以降Web版・モバイル版の一般ユーザー向けアカウントへ順次展開している。従来の「1日ごとにリセット」される固定回数制から、**5時間ごとにリセットされ、週単位の上限に達するとそこで止まる、コンピューティング量(計算資源)ベースの変動制**に切り替わった。プロンプトの複雑さ・チャットの長さ・ソースの数・使用する機能によって消費量が変わる仕組みで、無料(Standard)ユーザーを基準に、Google AI Plusは2倍、Google AI Proは4倍、Google AI Ultraは契約内容に応じて5倍または20倍の利用枠が割り当てられる。「今月は残り◯回」という単純な数え方ができなくなったため、大量のソースや長いチャットを扱う予定がある場合は、事前に上位プランでの利用枠を確認しておくとよい。
...
| プラン | 日本向け目安月額(税込) | 想定利用者 | ソース数・チャット/Studio利用枠の目安 |
|---|---|---|---|
| Standard(無料) | 0円 | 個人 | 1ノートブックあたりソース50件。チャット・Studio利用枠は基準倍率(1倍)、5時間ごとにリセット |
| Google AI Plus | 725円 | 個人・ライトユーザー | ソース100件程度まで拡大。利用枠は基準の2倍 |
| Google AI Pro | 2,900円 | 個人・仕事利用のヘビーユーザー | ソース300件程度、ノート内コード実行などが標準機能化。利用枠は基準の4倍 |
| Google AI Ultra 5x | 14,500円 | 高頻度利用の個人・チーム | ソース500〜600件程度。利用枠は基準の5倍 |
| Google AI Ultra 20x | 32,000円 | 大容量・最大級の利用が必要な個人 | Ultra 5xよりさらに上限が大きく、利用枠は基準の20倍。Cinematic Video Overviewなど最新機能・リアルタイム音声対話への先行アクセス |
...
上記の「利用枠の倍率」は前述の2026年9月2日改定によるコンピューティング量ベースの新方式(5時間ごとにリセット、週単位の上限あり)の倍率であり、旧来の「1日◯回」という固定回数の目安ではない点に注意。
...
- **利用上限が「1日◯回」から「5時間ごとの変動制」に変わった**: 2026年9月2日の改定以降、チャット・Studio機能の利用上限はプロンプトの複雑さやソース数に応じて消費量が変わるコンピューティング量ベースの方式になった。従来のように「今日はあと何回使えるか」を単純に数えられなくなったため、大量のソースを扱う長時間の作業を予定している場合は、事前に上位プランへの切り替えを検討する
...
### 2026-09-23: 利用上限方式の変更、モバイル新機能5種を反映して最新化
- **内容**: 2026年9月2日以降、チャット・Studioの利用上限が「1日ごとの固定回数制」から「5時間ごとにリセットされるコンピューティング量ベースの変動制」に変わったこと(Standardを基準にPlus 2倍・Pro 4倍・Ultra 5〜20倍)を反映し、料金プラン表・注意点を更新。2026年9月15日発表のモバイル新機能5種(約100言語対応のリアルタイム音声対話、ボイスレコーダー、図解・クイズ・フラッシュカード入り「学習の概要」、新クイズ形式、80言語以上に拡大した60秒の短尺動画概要)を仕組み・背景に追加
- **出典**: [窓の杜「『Gemini Notebook』の利用上限が変更、5時間ごとにリセット ~9月2日より展開」](https://forest.watch.impress.co.jp/docs/news/2137172.html)
- **出典**: [gihyo.jp「Gemini Notebook、モバイルアプリに音声対話や録音機能を順次追加へ」](https://gihyo.jp/article/2026/09/gemini-notebook-study-updates)

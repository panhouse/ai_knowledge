...
### 料金プラン(2026年9月時点、公式サイトの表記に基づく目安)

| プラン | 月額目安 | 主な内容 |
|---|---|---|
| Free | 無料 | 公開モデル・データセット・Spacesの閲覧・利用、標準的な保存容量・計算量の範囲内での利用 |
| PRO | 9ドル/月 | 非公開ストレージ容量10倍・公開ストレージ容量2倍、推論(Inference)クレジットがFreeの20倍、ZeroGPU利用枠が最大かつ優先度も最高、ZeroGPU/Gradio/DockerのSpacesをホスト可能、個人ブログの公開、非公開データセットのDataset Viewer利用など |
| Team | 20ドル/ユーザー・月 | SSO(SAML・OIDC)、Storage Regionsによるデータ保存地域の指定、監査ログ、詳細なアクセス権限・認証ポリシー。メンバー全員がZeroGPU・Inference Providerの特典を利用可能 |
| Enterprise | 50ドル/ユーザー・月 | Teamプランの内容に加え、ストレージ・帯域・API利用回数の上限が最高水準、SCIMによるユーザー管理の自動化、専任サポート |

2026年に入り、PRO以上のプランには「推論クレジット(inference credits)」という、モデル呼び出し量に応じた専用の消費枠が明示的に設けられている(PROはFreeの20倍のクレジットが付与される)。これとは別に、ストレージ($8〜12/TB/月、AWS S3の23ドルやBackblazeの15ドルより安価をうたう)、Spacesの有料GPU(CPUは無料枠あり、GPUは時間0.4ドル程度〜23.5ドル程度)、Inference Endpoints(CPUインスタンスは時間0.033ドルから、GPUは最大時間74ドル程度)という、利用量に応じた従量課金が別建てで発生する。閲覧・ダウンロードだけであれば無料の範囲で完結する。料金・クレジット制度は変更されやすいため、実際の利用前に[Hugging Face公式の料金ページ](https://huggingface.co/pricing)で最新の金額を確認すること。

## 注意点・よくある誤解

- **2026年に「推論クレジット」制度が明示化され、無料範囲の感覚が変わっている**: 以前は「ZeroGPUの共用枠」といった曖昧な説明だった無料の推論利用枠が、2026年からは「推論クレジット」という数値で管理されるようになった(PROはFreeの20倍)。従来の感覚のまま大量に推論APIを呼び出すと、想定より早く無料枠を使い切ったり追加課金が発生したりする可能性があるため、本格的に使う前にダッシュボードでクレジットの消費状況を確認する
...
### 2026-09-16: 料金プランを公式サイトの最新表記に更新
- **内容**: 公式の料金ページに基づき、PRO/Team/Enterpriseの内容を「非公開ストレージ10倍・公開ストレージ2倍・推論クレジット20倍」等の具体的な倍率表記に更新し、2026年から明示的に導入された「推論クレジット(inference credits)」制度を追記。ストレージ($8〜12/TB/月)・SpacesのGPU時間課金(0.4〜23.5ドル)・Inference Endpoints(CPU時間0.033ドル〜、GPU最大時間74ドル程度)の従量課金の目安も公式サイトの表記に合わせて更新。注意点に、無料の推論利用枠が数値管理化されたことによる消費ペースの見落としリスクを追加
- **出典**: [Hugging Face Pricing(公式)](https://huggingface.co/pricing)

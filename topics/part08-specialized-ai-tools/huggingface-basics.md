...
これらはすべて「リポジトリ」という単位で管理されており、GitHubのように誰でも公開・複製(fork)・改良版のアップロードができる。企業やコミュニティが新しいモデルを発表する際、まずHugging Face上にモデルカードとともに公開する、というのが今や業界の標準的なお披露目の場になっている。
...
| PRO | 9ドル/月程度 | 非公開リポジトリの追加容量、優先的な計算リソース枠など個人向けの拡張 |
| Team | 20ドル/ユーザー・月程度 | チームでの共同作業向け機能 |
| Enterprise | 50ドル/ユーザー・月以上 | シングルサインオン(SSO)・監査ログなど組織のガバナンス機能 |

これとは別に、Spacesで有料GPUを使う場合(時間課金)や、モデルをクラウド上でAPIとして動かす「Inference Endpoints」を使う場合は、利用量に応じた従量課金が発生する。閲覧・ダウンロードだけであれば無料の範囲で完結する。料金は変更されやすいため、実際の利用前に[Hugging Face公式の料金ページ](https://huggingface.co/pricing)で最新の金額を確認すること。

...
名称の変遷は、2023年7月に「Code Interpreter」としてベータ公開され、同年8月に「Advanced Data Analysis」へ改称、現在のChatGPTのヘルプセンターでは単に「データ分析(Data Analysis)」というツール名で案内されている。GPT-4o以降のモデルでは、ユーザーがツールを手動で選ばなくても、ファイルが添付され分析が必要と判断されると自動的にPythonコードの生成・実行に切り替わる(2026年8月時点の主力モデルはGPT-5.6ファミリー(Sol・Terra・Luna)。7月9日に正式提供が始まり、旧世代のGPT-5.4は同月23日に廃止された。さらに8月6日には、Plus・Pro向けにSolが更新され、応答にどれだけ考える時間をかけるかを選べる「thoughtスライダー」が追加されたほか、財務・医療・法律など正確さが問われる質問での事実誤りが旧モデル(GPT-5.5 Instant)比で68%減ったとOpenAIは説明している。無料・Goプランのデフォルトモデルも軽量版のLunaに切り替わったが、いずれの世代でもデータ分析の自動起動という挙動自体は変わらない)。回答の中に生成されたコードが表示されるため、「何を根拠にその数字が出たか」を確認できる点が、単なる自然言語での要約と異なる大きな特徴である。なお、GPTストアには「Data Analyst」という名前のカスタムGPTも存在するが、標準搭載のデータ分析機能とは別物なので混同しないこと。

2026年6月には、生成されるグラフの一部が「インタラクティブチャート」に対応した。棒グラフ・折れ線グラフ・円グラフ・散布図の4種類は、生成後に表示される「インタラクティブチャートに切り替える」を選ぶと、グラフ上の要素にカーソルを合わせて数値を確認したり、色をプリセットまたは16進数(hex)カラーコードで指定し直したりできる。ヒストグラム・箱ひげ図・ヒートマップ・エリアチャート・レーダーチャート・ツリーマップ・バブルチャート・ウォーターフォールチャートなど、その他の形式は引き続き画像として出力される。いずれのグラフもPNG形式でダウンロードできる。
...
1. `chatgpt.com` にログインし、チャット入力欄左側の「+」ボタンをクリックして「写真とファイルを追加」を選ぶ(バージョンによってはクリップアイコン📎がそのまま表示される場合もある)
...
6. アップロードしたファイルとChatGPTが生成したファイルは、自動的に画面左サイドバーの「ライブラリ」タブにも保存される。翌日以降、別の会話で同じファイルを使いたいときは、「+」ボタンから「ライブラリから追加」を選べば再アップロードせずに呼び出せる(2026年3月導入。Plus・Pro・Businessで利用可能、Free・Goにも順次拡大中)
...
- 2025年に日本でも提供が始まった低価格プラン「ChatGPT Go」は、2026年1月30日以降に日本円建ての固定価格(月額1,400円)となった。無料プランの10倍相当のファイルアップロード枠でデータ分析機能を使え、Plusの月額3,000円まで出せない場合の現実的な入口になる
...
- **継続利用が前提の資料には向かない**: 毎週更新するようなダッシュボードは、ライブラリのおかげでファイルの再アップロードこそ不要になったものの、集計・グラフ化の指示は会話ごとにやり直しになり自動更新はされない。BIツールやExcelの自動更新機能に任せた方がよい。また、デフォルトのグラフは配色・体裁が簡素なため、棒・折れ線・円・散布図であれば生成後に色をプリセットやhexコードで変更できるものの、それ以外の形式や凡例・タイトルの作り込みまでは自動で仕上がらない。社外向け資料に使う場合は色・凡例・タイトルを追加で指示するか、出力後に他のツールで整える
...
### 2026-08-15: インタラクティブチャート・GPT-5.6の8月更新・ChatGPT Go価格を反映して最新化
- **内容**: 2026年6月に追加された、棒・折れ線・円・散布図をホバーや色変更(プリセット/hexコード)ができる「インタラクティブチャート」に切り替えられる機能を反映し、グラフ出力に関する記述を更新した。あわせて、8月6日にPlus・Pro向けに行われたGPT-5.6 Solの更新(応答の思考量を選べるthoughtスライダー、事実誤りの削減)と無料・GoプランのデフォルトモデルがLunaになった点、ChatGPT Goの日本円建て価格が2026年1月30日以降1,400円に固定された点を反映した。ファイルアップロード操作の説明を、現行UIの「+」ボタン起点の手順に更新した
...
- **出典**: [OpenAI updating ChatGPT with a smarter GPT-5.6 Sol and unlimited free chats | 9to5Mac](https://9to5mac.com/2026/08/06/openai-updating-chatgpt-with-a-smarter-gpt-5-6-sol-and-unlimited-free-chats/)
- **出典**: [Extracting Insights with ChatGPT Data Analysis | OpenAI Help Center](https://help.openai.com/en/articles/9213685-extracting-insights-with-chatgpt-data-analysis)
- **出典**: [ChatGPT料金一覧｜Plus 3,000円・Go 1,400円・Pro 16,800円【2026年8月最新】 | AI革命株式会社メディア](https://ai-revolution.co.jp/media/chatgpt-pricing/)

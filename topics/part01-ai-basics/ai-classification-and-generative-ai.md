...
実務では両者が組み合わさることも多い。たとえば社内チャットボットは、ユーザーの質問意図を機械学習(識別系)で把握し、回答文そのものは生成AIで作る、という役割分担になっている。2026年時点でも「ルールベース+機械学習+生成AI(LLM)を組み合わせ、ルールで担保する透明性と機械学習・生成AIの柔軟性をどうバランスさせるか」が実務での基本的な設計思想であり、3者は対立する選択肢ではなく併用が前提になっている。なお、この「AI⊃機械学習⊃ディープラーニング⊃生成AI」という分類骨子は、日本ディープラーニング協会(JDLA)のG検定シラバスでも2026年時点の版で維持されており、業界標準として通用する整理と考えてよい。

さらに2025年後半以降は、生成AI(LLM)を土台にしつつ「目標(ゴール)だけを渡せば、手順の計画・外部ツールの呼び出し・結果確認までを自律的にループさせる」**エージェントAI(エージェンティックAI)**という発展形が実務に広がっている。生成AIが「質問に答える」「文章を生成する」という受動的な役割にとどまるのに対し、エージェントAIは与えられた目的に応じて自律的に判断・行動し、複数のタスク・外部システム連携を統合的に遂行する能動的な役割を担う点が異なる。この整理は日本政府(総務省・経済産業省)が2026年3月31日に公表した「AI事業者ガイドライン(第1.2版)」で、AIエージェントおよびフィジカルAIの定義・便益・リスクとして正式に追記された内容とも一致する。両者は同じ生成AI(LLM)の技術を使いながら「振る舞い方」で区別される点は変わらない。詳細は[AIエージェントとは何か](../part11-ai-agents/ai-agent-basics.md)を参照。
...
| モダリティ | 代表的なサービス例(2026年8月時点) |
|---|---|
| テキスト生成 | ChatGPT(OpenAI、GPT-5.6系列: Sol/Terra/Luna。2026年7月9日に一般提供開始、高速な既定モデルとしてGPT-5.5 Instantも併存)、Claude(Anthropic、Sonnet 5/Opus 4.8/Fable 5)、Gemini(Google、Flash/Pro) |
...
- **モダリティ別の代表ツールは半年〜1年でも入れ替わる**: OpenAI Soraのように2026年に入って提供終了が決まったサービスもあれば、ChatGPTのGPT-5.5→GPT-5.6やGoogle「Nano Banana」シリーズのように、同一ブランド内でモデルが数か月おきに更新されるケースもある。社内資料・研修教材に固有のツール名・モデル名を書いたら、半年に一度は現行かどうかを見直す運用にする
...
### 2026-08-05: エージェントAIの位置づけに日本政府の公式定義を反映し、テキスト生成の代表例をGPT-5.6系列に更新
- **内容**: 「AI⊃機械学習⊃ディープラーニング⊃生成AI」の分類骨子が2026年時点のJDLA G検定シラバスでも維持されていることを明記。エージェントAI(エージェンティックAI)の説明を、2026年3月31日公表の「AI事業者ガイドライン(第1.2版)」(総務省・経済産業省)によるAIエージェントの正式定義(生成AIの受動的な役割に対し、目的に応じて自律的に判断・行動し複数タスク・外部システム連携を統合的に遂行する能動的な役割)に沿って更新し、出典を差し替え。テキスト生成の代表例をChatGPTのGPT-5.6系列(Sol/Terra/Luna、2026年7月9日一般提供開始)に更新し、GPT-5.5 Instantの併存にも言及。画像・動画・音声生成の代表例(Nano Banana Pro/2/2 Lite、Veo 3.1、Runway Gen-4.5、Kling 3.0、ElevenLabs、Suno/Udio)は2026年8月5日時点でも現行であることをWeb検索で再確認した
- **出典**: [総務省・経済産業省: AI事業者ガイドライン(第1.2版)](https://www.meti.go.jp/shingikai/mono_info_service/ai_shakai_jisso/pdf/20260331_1.pdf)、[PwC Japan: 「AI事業者ガイドライン(第1.2版)」改定のポイントと事業者への期待](https://www.pwc.com/jp/ja/knowledge/column/ai-governance/ai-guideline-03.html)、[日本ディープラーニング協会(JDLA): G検定について](https://www.jdla.org/certificate/general/)、[CNBC: OpenAI to publicly release GPT-5.6, rolls out conversational AI models](https://www.cnbc.com/2026/07/08/openai-expanding-gpt-5point6-ai-model-release-ending-government-limits.html)、[Axios: OpenAI releases GPT-5.6 and ChatGPT Work tool](https://www.axios.com/2026/07/09/ai-openai-gpt-release)、[Google DeepMind: Gemini 3.1 Flash-Lite Image – Nano Banana 2 Lite](https://deepmind.google/models/gemini-image/flash-lite/)

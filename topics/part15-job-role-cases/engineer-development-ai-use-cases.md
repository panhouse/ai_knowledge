...
tags: [ソフトウェア開発, コードレビュー, DORA, セキュリティ, 技術的負債, スキル低下, GitHub Copilot, 導入事例]
created: 2026-08-06
updated: 2026-09-22
---
...
コーディング支援AIは「試しに使ってみた」段階を超え、全社ガバナンスを敷いて定点観測する企業が増えている。ここでは、実名で公表された効果指標を持つ事例を、公表主体(自社公表/ベンダー公表)を明記したうえで紹介する。数値は必ず時点付きで読み、他社にそのまま当てはまるとは限らない点に注意する。

### NTTドコモ(通信キャリア大手) — 対象業務: ソフトウェア開発全般(コード実装)
- **導入形態**: 専用SaaS導入(GitHub Copilot Enterprise)
- **段階**: 全社展開(GitHub Enterprise利用者6,012名中、Copilot登録ユーザー3,039名。2025年9月時点)
- **やったこと**: 全社的な利用ガバナンスポリシーを策定したうえでGitHub Copilotを展開し、約3ヶ月間(2025年6月8日〜9月15日)のコード提案・採用実績をダッシュボードで可視化して定点観測する運用を確立した。
- **効果**: 総提案数213万1,503件のうち51万7,524件を採用(採用率24.28%)。「1件の採用コードを1分の工数短縮」と仮定した試算で、月平均約20人月相当の削減効果(2025年9月時点、自社公表・自社開発者ブログ)。
- **学べること**: 「採用率」を継続的に定点観測する仕組みを持つと、生産性換算の妥当性を検証しながら展開規模を判断できる。
- **出典**: [NTTドコモ ENGINEERING BLOG「NTTドコモのGitHub Copilot導入事例と運用ポリシー」](https://nttdocomo-developers.jp/entry/docomo-github-copilot-governance) / 最終確認日: 2026-09-22

### 日立製作所(総合電機・ITサービス大手) — 対象業務: アプリケーション開発(業務ロジック実装)
- **導入形態**: 専用SaaS導入(GitHub Copilot)
- **段階**: 部門展開から全社展開へ拡大中(2023年10月に約200名で社内評価を開始、2024年4月に「生成AI実務者コミュニティ」発足、2025年3月時点で利用者約2,000名、5,000名まで拡大予定)
- **やったこと**: アプリケーションサービス事業部を中心に、自社の開発フレームワーク「Justware」とGitHub Copilotを併用する形で導入し、活用ノウハウを共有する社内コミュニティを運営している。
- **効果**: 社内アンケートで「タスクを迅速に完了できる」との回答が83%。生産性はケースにより10〜20%、一部で30%向上。業務ロジックのコード生成率はフレームワーク単体の78%からCopilot併用で99%に向上(2025年3月時点、ベンダー公表・Microsoft導入事例ページ)。
- **学べること**: 独自フレームワークとAIを組み合わせる場合、「フレームワーク単体でどこまでできるか」を基準値に置くと効果を測定しやすい。
- **出典**: [Microsoft Customer Stories「日立製作所が GitHub Copilot の活用で開発生産性を向上」](https://www.microsoft.com/ja-jp/customers/story/22781-hitachi-github) / 最終確認日: 2026-09-22

### ソフトバンク(通信キャリア大手) — 対象業務: 開発が本業でないメンバーによるサービス開発(TASUKI Annotation)
- **導入形態**: 専用SaaS導入(GitHub Copilotのカスタムコマンド機能)
- **段階**: PoC(開発が本業ではないメンバー2名のプロジェクトチームでの検証。全社数値ではない)
- **やったこと**: GitHub Copilotを「個人の道具」から「チームの脳」にする狙いで、レビュー観点やコーディング規約をチーム共通のカスタムコマンドとして整備し、開発が本業でない2名のチームに適用した。
- **効果**: 対象チームの開発工数を34%削減。内訳は関数の説明文生成94%減、ログ整備95%減、ユニットテスト整備73%減(2025年12月時点、自社公表・自社テックブログ。2名チームでの実測値であり全社の成果ではない)。
- **学べること**: チーム独自のレビュー観点をカスタムコマンド化すると、開発経験の浅いメンバーでも定型作業の削減効果を再現しやすい。
- **出典**: [ソフトバンク クラウドテクノロジーブログ「GitHub Copilotを『個人の道具』から『チームの脳』へ」](https://www.softbank.jp/biz/blog/cloud-technology/articles/202512/github-copilot-custom-command/) / 最終確認日: 2026-09-22

### 事例から見える傾向

3社に共通するのは、「導入して終わり」ではなく**利用データを継続的に可視化する運用**を敷いている点である。NTTドコモは採用率、日立製作所はアンケート評価とコード生成率、ソフトバンクはタスク種別ごとの削減率と、いずれも定量指標をダッシュボード化・可視化してから展開規模を判断している。また日立製作所とソフトバンクの事例が示すように、既存の開発フレームワークやチーム独自のレビュー観点とAIを組み合わせる「併用型」の導入の方が、単純な生成コード量の増加よりも実務上の効果測定がしやすい傾向にある。一方でPoC・部門実証段階の数値(ソフトバンクの2名チームなど)を全社の成果であるかのように読み違えないよう、各社が明記する対象範囲を必ず確認する必要がある。なお本ページの「実務での使い方」で紹介するメルカリ・LayerX・DMMの事例もあわせて参照されたい。
...
- **内容**: 「使いどころ・使い分け」の後に「導入事例カタログ」節を新設し、NTTドコモ・日立製作所・ソフトバンクの3社によるGitHub Copilot導入事例(採用率・生産性向上率・工数削減率と時点・公表主体つき)をcase-card形式で追加。frontmatterのtagsに「GitHub Copilot」「導入事例」を追加
- **出典**: [NTTドコモ ENGINEERING BLOG](https://nttdocomo-developers.jp/entry/docomo-github-copilot-governance), [Microsoft Customer Stories: 日立製作所](https://www.microsoft.com/ja-jp/customers/story/22781-hitachi-github), [ソフトバンク クラウドテクノロジーブログ](https://www.softbank.jp/biz/blog/cloud-technology/articles/202512/github-copilot-custom-command/)

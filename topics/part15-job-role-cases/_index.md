---
title: "Part 15: 職種別 生成AI活用事例"
part: 15
---

# Part 15: 職種別 生成AI活用事例

**職種(ジョブファンクション)ごと**に、生成AIが実際にどう使われているかを整理して集約する場所。
Part 14(業種別)と対になる事例カタログで、業種を横断して
「営業なら何をやっているか」「経理なら何をやっているか」を引ける状態に育てる。

もとは Part 12 第5章「職種別ユースケース」として書き溜めていたが、
ページ数が増えて Part 12(手法の教科書)の一章に収まらなくなったため、独立したパートに切り出した。

## 他パートとの住み分け

| パート | 切り口 | 何を書くか |
|---|---|---|
| Part 12(業務活用・実践) | 手法・考え方 | 文章作成、資料作成、リサーチなど**職種を問わないタスク単位の教科書** |
| Part 14(業種別事例) | 業種 | 製造業・金融など**その業界で何が起きているか** |
| **Part 15(職種別事例)** | **職種** | **その職種の業務フローに沿った具体的な活用パターンと事例** |
| Part 16(国内AI企業マップ) | 企業 | そのツールを**どの会社が提供しているか**(企業ディレクトリ) |

- 職種を問わない手法に整理できる内容は Part 12 に書き、
  特定職種の業務フローに踏み込む内容はこの Part 15 に書く
- 同じネタが Part 14 と重なる場合(例: 人事職 と 人材サービス業)は、
  **主となる切り口の章に置き、もう一方の章から相互リンクする**
- ツールの提供元企業に触れるときは深掘りせず、Part 16 の該当ページへリンクする

## 書き方の指針(このパート固有)

雛形は **`templates/job-role-case-page.md`**(Part 15 専用。教科書用の `topic-page.md` は使わない)。
章の単位は職種、ページの単位は原則「1職種1ページ」とし、内容が育ったら業務単位に分割する。

このパートは「使い方ガイド」ではなく**事例カタログ**である。
各ページには次の5点を必ず入れ、**「導入事例カタログ」を主役**にする。

1. **業務フェーズ別の活用マップ** — その職種の仕事の流れに沿って、どこにAIが入るか
2. **導入事例カタログ** — 実名の組織による導入事例を、雛形の「事例カード」1事例1枚で書く
3. **使えるツール・構成** — 汎用チャットAIで足りる部分と、専用ツールが必要な部分の切り分け
4. **コピペで使えるプロンプト例** — そのまま試せる形で全文を載せる
5. **その職種特有の落とし穴** — 個人情報、法規制、数値の裏取り、独立性など

### 導入事例カタログのルール

カードの書式と必須メタデータ(導入形態/段階/効果の時点と公表主体/最終確認日/出典)は
**`templates/case-card.md` が正**。Part 14(業種別)と共通の書式を使う。以下はこのパート固有の補足。

- **件数**: 新規執筆は実在事例 **5件以上**、既存ページの最新化・増強は **3件以上の追加** を目標にする。
  件数を満たせない職種は無理に書かず次回に回す(水増し禁止。想定シナリオで件数を埋めない)
- **実名**: 組織名は実名で書く。「大手製造業A社」のような匿名事例は載せない
- **効果の表記**: 数値には必ず「時点」と「公表主体」を付ける。
  公表主体は **ベンダー公表 / 導入企業の自社公表 / 第三者取材(メディア・省庁・学会)** の3区分
- **導入形態と段階**: 各事例に「汎用チャットAI活用 / 専用SaaS導入 / 内製」と
  「PoC / 部門展開 / 全社展開 / 不明」を付ける(記事側で「どの段階の事例か」を選別できるようにする)
- **実在事例と活用パターンを混ぜない**: 「導入事例カタログ」節には実在事例だけを書き、
  読者が今日から試せる汎用の使い方・プロンプト例は「実務での使い方(活用パターン)」節に分けて書く
- **Part 14 との重複**: 同じ事例が業種ページにもふさわしい場合は、主となる切り口の側に本文を置き、
  もう一方には1行リンクだけを置く(例: 銀行員 ↔ 金融・保険業、薬剤師 ↔ 調剤薬局業界)
- **最終確認日**: 各カードに `最終確認日: YYYY-MM-DD` を持たせ、最新化のときに再確認する

### 事例の探し先

- 導入企業のプレスリリース(PR TIMES・自社ニュースリリース)、統合報告書・IR資料・決算説明資料
- ベンダーの導入事例ページ(必ず「ベンダー公表」と表記)
- 業界メディア・専門紙(日経クロステック、ITmedia、業界紙、職能団体の機関誌)
- 省庁・自治体・公的機関の事例集(デジタル庁、経済産業省、IPA、総務省、厚生労働省)
- 職能団体・学会の発表(看護協会、税理士会、各学会の発表資料など)
- 海外は企業テックブログ、HBR、ベンダーのカスタマーストーリー(海外事例と明記)

### その他の指針

- 一般論の「営業でもAIが使えます」で終わらせない。**業務名まで下ろす**
- 隣接職種と内容が重なるときは、境界線(何を扱い何を扱わないか)を冒頭で明記する
  (例: 広報・PR職ページはマーケティング職ページとのすみ分けを書く)
- 既存ページ(この雛形より前に書かれたもの)は、最新化の対象になったタイミングで
  「導入事例カタログ」節を追加し、順次この雛形の構成に寄せていく

## 章構成(分類の地図)

### 第1章 職種横断・総論
- 複数の管理部門をまとめて見渡す総論

### 第2章 営業・セールス
- 商談準備・提案・フォロー・CRM入力

### 第3章 マーケティング・広報
- コンテンツ・広告・SNS・プレスリリース・メディア対応・IR

### 第4章 カスタマーサポート・カスタマーサクセス
- 一次対応・FAQ・応対品質・ヘルススコア・解約予兆

### 第5章 企画・プロダクト・データ分析
- 事業企画、PdM(PRD・要求整理)、データ分析・BI

### 第6章 人事・採用・人材開発
- 採用、労務、評価、研修・L&D

### 第7章 経理・財務・内部監査
- 証憑処理、月次・決算、予実分析、監査

### 第8章 法務・知財・コンプライアンス
- 契約レビュー、法令調査、社内FAQ

### 第9章 総務・秘書・購買調達
- 社内対応、日程・会議運営、RFP・サプライヤー管理

### 第10章 情報システム・情報セキュリティ
- 社内ヘルプデスク、障害対応、シャドーAI管理、SOC業務

### 第11章 エンジニア・開発
- コード生成・レビュー、テスト、仕様書・設計書作成、障害調査、レガシー解析
- ツール自体の解説は Part 8「コーディング支援AI」にあるため、ここでは
  **開発者の作業フローのどこに組み込むか**と、レビュー・品質担保の運用を書く

### 第12章 研究開発・品質管理
- 文献・特許調査、実験計画、QA(ソフトウェア/製造)

### 第13章 生産管理・サプライチェーン
- 需要予測、生産計画、サプライヤーリスク

### 第14章 デザイナー・クリエイティブ
- ムードボード・ラフ・バリエーション生成、ワイヤーフレーム、マイクロコピー、
  フィードバック整理、素材の権利確認

### 第15章 翻訳・通訳
- 機械翻訳のポストエディット、用語集・スタイルガイドの運用、
  リアルタイム通訳、字幕・吹き替え

### 第16章 コンサルタント・リサーチャー
- 仮説構築、Deep Researchによる調査、インタビュー分析、成果物のドラフト
- 社内の企画職(第5章)との違いは「他社に成果物として納品する」前提。
  出典明示と裏取りの厳しさをどう担保するかを軸に書く

### 第17章 経営者・管理職
- 意思決定材料の整理、社内メッセージ作成、レポート読み込み、1on1・評価の準備

### 第18章 その他の職種
- 上記に当てはまらない職種の一時受け皿

## 収録ページ

### 第1章 職種横断・総論
- [バックオフィス職における生成AI活用](backoffice-ai-use-cases.md)

### 第2章 営業・セールス
- [営業職における生成AI活用](sales-ai-use-cases.md)
- [不動産営業(仲介・売買)における生成AI活用事例](real-estate-agent-ai-use-cases.md)
- [保険営業(生命保険・損害保険)における生成AI活用事例](insurance-agent-ai-use-cases.md)
- [製薬MR(医薬情報担当者)における生成AI活用事例](pharma-mr-ai-use-cases.md)

### 第3章 マーケティング・広報
- [マーケティング職における生成AI活用](marketing-ai-use-cases.md)
- [広報・PR職における生成AI活用事例](pr-communications-ai-use-cases.md)

### 第4章 カスタマーサポート・カスタマーサクセス
- [カスタマーサポート職における生成AI活用事例](customer-support-ai-use-cases.md)
- [カスタマーサクセス職における生成AI活用事例](customer-success-ai-use-cases.md)

### 第5章 企画・プロダクト・データ分析
- [企画職における生成AI活用](planning-ai-use-cases.md)
- [プロダクトマネージャー(PdM)職における生成AI活用事例](product-manager-ai-use-cases.md)
- [データアナリスト/BIアナリスト職における生成AI活用事例](data-analyst-ai-use-cases.md)

### 第6章 人事・採用・人材開発
- [人事(HR)職における生成AI活用事例](hr-ai-use-cases.md)
- [人材育成・研修(L&D)担当における生成AI活用事例](learning-development-ai-use-cases.md)
- [労務・給与担当者における生成AI活用事例](payroll-labor-administration-staff-ai-use-cases.md)

### 第7章 経理・財務・内部監査
- [経理・財務職における生成AI活用事例](finance-accounting-ai-use-cases.md)
- [内部監査職における生成AI活用事例](internal-audit-ai-use-cases.md)
- [税理士における生成AI活用事例](tax-accountant-ai-use-cases.md)
- [公認会計士・監査法人職員における生成AI活用事例](cpa-external-auditor-ai-use-cases.md)

### 第8章 法務・知財・コンプライアンス
- [法務職における生成AI活用事例](legal-ai-use-cases.md)
- [司法書士・行政書士における生成AI活用事例](judicial-administrative-scrivener-ai-use-cases.md)
- [社会保険労務士(社労士)における生成AI活用事例](labor-social-security-consultant-ai-use-cases.md)
- [知財・特許担当者における生成AI活用事例](ip-patent-ai-use-cases.md)

### 第9章 総務・秘書・購買調達
- [総務職における生成AI活用事例](general-affairs-ai-use-cases.md)
- [秘書・エグゼクティブアシスタント職における生成AI活用事例](executive-assistant-ai-use-cases.md)
- [購買・調達職における生成AI活用事例](procurement-ai-use-cases.md)

### 第10章 情報システム・情報セキュリティ
- [情報システム(IT)部門における生成AI活用事例](information-systems-ai-use-cases.md)
- [情報セキュリティ担当者(SOC/CISO)における生成AI活用事例](information-security-ai-use-cases.md)

### 第11章 エンジニア・開発
- [エンジニア・開発職における生成AI活用事例](engineer-development-ai-use-cases.md)
- [SRE・インフラエンジニアにおける生成AI活用事例](sre-infrastructure-engineer-ai-use-cases.md)
- [データエンジニアにおける生成AI活用事例](data-engineer-ai-use-cases.md)
- [ITプロジェクトマネージャー(システム開発PM)における生成AI活用事例](it-project-manager-ai-use-cases.md)

### 第12章 研究開発・品質管理
- [研究開発(R&D)職における生成AI活用事例](rd-ai-use-cases.md)
- [品質管理・QA職における生成AI活用事例](quality-assurance-ai-use-cases.md)
- [大学教員・研究者における生成AI活用事例](university-faculty-researcher-ai-use-cases.md)
- [臨床開発・治験担当者(CRA・CRC)における生成AI活用事例](clinical-trial-cra-crc-ai-use-cases.md)

### 第13章 生産管理・サプライチェーン
- [生産管理・SCM(サプライチェーンマネジメント)担当における生成AI活用事例](production-management-scm-ai-use-cases.md)
- [倉庫管理・在庫管理担当者における生成AI活用事例](warehouse-inventory-manager-ai-use-cases.md)
- [生産技術者(プロセス・設備エンジニア)における生成AI活用事例](production-engineer-process-technology-ai-use-cases.md)

### 第14章 デザイナー・クリエイティブ
- [デザイナー・クリエイティブ職における生成AI活用事例](designer-creative-ai-use-cases.md)

### 第15章 翻訳・通訳
- [翻訳・通訳担当における生成AI活用事例](translation-interpretation-ai-use-cases.md)
- [字幕翻訳・ローカライズ担当における生成AI活用事例](subtitle-localization-specialist-ai-use-cases.md)

### 第17章 経営者・管理職
- [経営者・管理職における生成AI活用事例](executive-management-ai-use-cases.md)
- [中間管理職(マネージャー)における生成AI活用事例](middle-manager-ai-use-cases.md)
### 第16章 コンサルタント・リサーチャー
- [コンサルタント・リサーチャー職における生成AI活用事例](consultant-researcher-ai-use-cases.md)
- [市場調査アナリスト(マーケットリサーチャー)における生成AI活用事例](market-research-analyst-ai-use-cases.md)

### 第18章 その他の職種
- [銀行員(窓口・融資担当)における生成AI活用事例](bank-teller-loan-officer-ai-use-cases.md)
- [店舗・販売スタッフ職における生成AI活用事例](retail-store-staff-ai-use-cases.md)
- [教員・保育士における生成AI活用事例](teacher-childcare-worker-ai-use-cases.md)
- [ドライバー・配送スタッフにおける生成AI活用事例](driver-delivery-staff-ai-use-cases.md)
- [製造・建設現場の作業員・現場監督における生成AI活用事例](frontline-worker-supervisor-ai-use-cases.md)
- [不動産管理(プロパティマネジメント)担当者における生成AI活用事例](property-management-ai-use-cases.md)
- [人材紹介エージェント(キャリアアドバイザー)における生成AI活用事例](recruiting-agent-career-advisor-ai-use-cases.md)
- [理学療法士・作業療法士(リハビリ専門職)における生成AI活用事例](pt-ot-rehabilitation-therapist-ai-use-cases.md)
- [ケアマネジャー(介護支援専門員)における生成AI活用事例](care-manager-ai-use-cases.md)
- [栄養士・調理師における生成AI活用事例](dietitian-cook-ai-use-cases.md)
- [獣医師における生成AI活用事例](veterinarian-ai-use-cases.md)
- [医療事務・クリニック受付職における生成AI活用事例](medical-office-clerk-ai-use-cases.md)
- [記者・編集者における生成AI活用事例](journalist-editor-ai-use-cases.md)
- [介護職員における生成AI活用事例](caregiver-eldercare-worker-ai-use-cases.md)
- [薬剤師における生成AI活用事例](pharmacist-ai-use-cases.md)
- [建築士・設計士における生成AI活用事例](architect-ai-use-cases.md)
- [金融アドバイザー・FP(ファイナンシャルプランナー)における生成AI活用事例](financial-advisor-ai-use-cases.md)
- [医師における生成AI活用事例](physician-ai-use-cases.md)
- [看護師・医療従事者における生成AI活用事例](nurse-medical-staff-ai-use-cases.md)
- [通関士・貿易事務担当者における生成AI活用事例](customs-broker-trade-operations-ai-use-cases.md)
- [美容師・理容師における生成AI活用事例](hairdresser-barber-ai-use-cases.md)
- [塾講師・家庭教師における生成AI活用事例](tutor-cram-school-instructor-ai-use-cases.md)

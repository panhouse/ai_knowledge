...
この3つのうち2つまでなら被害は限定的だが、3つが揃うと「悪意あるWebページを読んだだけで機密メールが外部に送信される」といった攻撃が成立する。AIエージェント(目標だけを渡せば自律的に複数ステップの作業をやり切るAIシステム。詳しくは[AIエージェントとは何か](../part12-ai-trends/ai-agent-basics.md))がWeb閲覧・メール処理・ファイル操作を人間の確認なしに任されるようになるほど、この3要素が同時に揃う場面が構造的に増えていく。

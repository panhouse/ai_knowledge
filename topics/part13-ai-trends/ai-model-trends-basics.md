...
### 主要ラボの現在地(2026年8月14日時点)

| ラボ | 現在の主力モデル | 補足 |
|---|---|---|
| OpenAI | **GPT-5.6ファミリー**(Luna/Terra/Sol の3段階。2026年6月26日に限定プレビュー→7月9日に一般提供が現行の最上位) | Solが最上位・最高価格帯。セキュリティ検証用途に特化した派生モデル「GPT-5.6-Cyber」も投入し、認可を受けた脆弱性診断(ゼロデイ探索等)向けに提供している |
| Anthropic | **Claude Opus 5**(7月24日発表。Fable 5の半額程度の価格帯で肩を並べる知性を持つ「日常使いの最上位モデル」と位置付け、Claude Maxの既定・Claude Proの最上位モデルに)、**Claude Sonnet 5**(6月30日発表。導入価格の$2/$10入出力を8月10日に恒久化し、9月1日に予定していた$3/$15への値上げを撤回)、**Claude Fable 5 / Mythos 5**(最上位。輸出規制で一時停止後、7月1日に世界展開再開) | Opus 5はコーディング・知識労働の評価(Frontier-Bench、GDPval-AA等)で自社史上最高を記録した一方、サイバーセキュリティ関連タスクではMythos 5に劣るとされる。8月にはOpus 4.1が提供終了となり、Opus 5への移行が案内された |
| Google | **Gemini 3.1 Pro**が現行の一般提供モデル。次期主力の**Gemini 3.5 Pro**は5月のGoogle I/Oで予告されたが、6月→7月17日→8月上旬の噂と延期が続き、8月14日時点でも一般提供に至っていない(Vertex AIでの企業向け限定プレビューのみ) | 7月21日には3.5 Proではなく「Gemini 3.6 Flash」「Gemini 3.5 Flash-Lite」「Gemini 3.5 Flash Cyber」という中位モデル群を先に投入しており、最上位モデルの遅延が長期化している |
| xAI | **Grok 4.6**(8月12日に一般公開。50万トークンの文脈window、長時間稼働するエージェント・コーディング・視覚タスクに重点) | Grok 4.5(7月8日公開)の後継。総合力ではGPT-5.6 Solと同水準、Claude Fable 5にわずかに劣るが、経済価値の高い業務タスク(GDPval-AA等)では上位モデルに勝る評価もある。6兆パラメータ級とされる次世代「Grok 5」はQ1→Q2と延期を重ね、8月時点でも正式な提供時期は未確定 |
| Meta | オープンウェイト(モデルの重みを公開し自社サーバーで動かせる方式)のLlama 4 Scout/Maverickは提供継続。一方でMeta Superintelligence Labsはコーディング特化の非公開モデル「Muse Spark 1.2」とターミナル型コーディングエージェント「Muse Code」を8月5日に投入 | Muse Spark 1.2は既存のNVIDIA以外にも展開予定。プロンプト・生成内容の学習提供と引き換えに大幅割引となる「contributor」料金プランを新設した点が特徴的。小型のオープンウェイトモデル「Muse Glimmer」(Apache 2.0)も近日公開予定と表明されている |
| Moonshot AI(中国) | **Kimi K3**(7月16日にAPI提供開始、7月27日に重み公開。2.8兆パラメータで過去最大級のオープンウェイトモデル) | Modified MIT ライセンスで公開。Artificial Analysis Intelligence Indexでは、独自モデルを除くオープンウェイトモデルの中で最上位クラスに位置し、コーディング系の一部評価では上位の独自モデルに匹敵するとの報告もある |
| DeepSeek(中国) | **DeepSeek V4 Pro/Flash**(4月24日プレビュー→7月中旬に正式提供→8月12〜13日に改訂版「V4 Pro 0813」がプレビューを終えて正式版に) | 総合スコア(Artificial Analysis Intelligence Index)自体は伸び悩んだとの評価がある一方、エージェント型コーディングやサイバーセキュリティ関連の評価スコアはプレビュー版から大きく上昇した。低価格路線を継続し、料金・時間帯別課金の枠組みも維持されている |

この一覧からわかる実務上のポイントは、「モデル名の枝番更新は数週間〜数か月ごとに動くのが常態であり、その都度乗り換えを検討する必要はない」ということと、「発表(アナウンス)と一般提供(GA)の間には数週間〜数か月のタイムラグがあり得る」ということである(Gemini 3.5 Proのように、発表から3か月以上経っても一般提供に至らない例もある)。乗り換えを検討すべきかどうかは、後述の「使いどころ・使い分け」で扱う判断軸で考える。
...
**主要ベンチマークの一覧(2026年8月時点でよく引用されるもの)**
...
| Artificial Analysis Intelligence Index | 複数のベンチマークを合成した総合指数(v4.0はGDPval-AA・Terminal-Bench・GPQA Diamond・SciCodeなど10種のベンチマークを合成) | 8月時点の版ではClaude Opus 5が首位(63.0%)、僅差でClaude Fable 5(62.1%)、GPT-5.6 SolとGrok 4.6がほぼ同水準(61%前後)で続く。Kimi K3はオープンウェイトモデルとして最上位クラスに入る。指数の版が更新されるたびに、どのテストをどう重み付けしたかという設計次第でランキングが動く点は個別ベンチマークと同じである |
...
### 主要モデルの価格帯(2026年8月時点、API料金・100万トークンあたり)
...
| Muse Spark 1.2(Meta、通常プラン) | $1.25 | 非公開(プロンプト等の学習提供に同意する「contributor」プランは$0.10/$0.20) | – |
| DeepSeek V4 Pro | $0.435(通常時。ピーク時間帯は約2倍) | $0.87(同上) | 100万トークン |
| DeepSeek V4 Flash | $0.14 | $0.28 | 100万トークン |

**業務での含意**: 最上位モデル(Fable 5・Opus 5・Sol)と廉価モデル(DeepSeek V4 Flash等)の間には価格で1桁〜2桁の差がある。すべての依頼を最上位モデルに投げるのではなく、「下書き・大量処理は廉価モデル、最終チェック・複雑な推論は上位モデル」という階層運用(モデルルーティング)が費用対効果を大きく左右する。GPT-5.6が3ティア構成を採ったのも、この使い分けを前提にした価格設計だと理解しておくとよい。また、Metaの「contributor」プランのように、自社のプロンプト・生成内容を学習データとして提供する代わりに大幅割引を受けられる料金プランも登場している点は、機密情報の取り扱いポリシーと合わせて確認すべきポイントである。
...
| API利用時(開発者向け) | リクエストに含めるモデルID(例: `gpt-5.6-sol`、`claude-opus-5`、`claude-fable-5`)を各社ドキュメントで確認。モデルIDは頻繁に増減するため、廃止予定(deprecation)のお知らせをAPIダッシュボードで定期確認する |
